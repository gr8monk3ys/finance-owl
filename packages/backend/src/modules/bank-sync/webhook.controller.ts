...
export class WebhookController {
  ...
  async handlePlaidWebhook(
    ...
    if (!isValid) {
      this.logger.warn(
        `Plaid webhook verification failed for item ${body.item_id}`,
      );
    ...
    switch (body.webhook_type) {
      ...
      default:
        this.logger.log(
          `Unhandled webhook type: ${body.webhook_type}/${body.webhook_code}`,
        );
    }
  ...
  private async handleTransactionWebhook(body: PlaidWebhookBody) {
    switch (body.webhook_code) {
      case 'SYNC_UPDATES_AVAILABLE':
        this.logger.log(
          `Transaction sync updates available for item ${body.item_id}`,
        );
        await this.transactionSyncScheduler.queueSyncForPlaidItemId(
          body.item_id,
          'webhook',
        );
        break;
      ...
      case 'HISTORICAL_UPDATE':
        this.logger.log(
          `Transaction ${body.webhook_code} for item ${body.item_id}`,
        );
        await this.transactionSyncScheduler.queueSyncForPlaidItemId(
          body.item_id,
          'webhook',
      ...
      default:
        this.logger.log(
          `Unhandled transaction webhook: ${body.webhook_code}`,
        );
    }
  }
  ...
  private async handleItemWebhook(body: PlaidWebhookBody) {
    switch (body.webhook_code) {
      case 'ERROR':
        ...
        await this.bankSyncService.updateItemStatus(
          body.item_id,
          'error',
      ...
      case 'LOGIN_REPAIRED':
        this.logger.log(
          `Login repaired for item ${body.item_id}`,
        );
      ...
      case 'USER_PERMISSION_REVOKED':
        this.logger.warn(
          `User revoked permission for item ${body.item_id}`,
        );
        await this.bankSyncService.updateItemStatus(
          body.item_id,
          'revoked',
      ...
      default:
        this.logger.log(
          `Unhandled item webhook: ${body.webhook_code}`,
        );
    }
  }
}
