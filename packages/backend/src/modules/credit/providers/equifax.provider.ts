...
export class EquifaxProvider implements CreditBureauProvider {
  ...
  async getCreditScore(userId: string, _ssn?: string): Promise<CreditScoreResult> {
    ...
    try {
      ...
      return {
        score: response.creditScore?.score || response.score,
        model: this.mapScoreModel(response.creditScore?.modelType),
        range: { min: 300, max: 850 },
        factors: this.mapFactors(response.creditScore?.factors || []),
        pulledAt: new Date(),
        bureau: 'equifax',
      };
    } catch (error) {
