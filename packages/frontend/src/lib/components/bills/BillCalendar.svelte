<script lang="ts">
  ...
  const selectedDayBills = $derived(selectedDate ? (billsByDate.get(selectedDate) ?? []) : []);
  ...
  const monthTotal = $derived(monthBills.reduce((sum, b) => sum + b.estimatedAmount, 0));
</script>
                ...
                style="background-color: {status === 'overdue'
                  ...
                  : bill.categoryColor
                    ...
                    : 'rgba(139, 92, 246, 0.12)'};
								color: {status === 'overdue' ? '#ef4444' : (bill.categoryColor ?? '#a78bfa')}"
