<script lang="ts">
  ...
  const netCashFlow = $derived(currentMonth ? currentMonth.income - currentMonth.spending : 0);
</script>
  ...
  {#if monthlyTrend.length > 0}
    {#if currentMonth}
      <div class="mt-2 flex items-baseline gap-2">
        <span class="text-lg font-bold {netCashFlow >= 0 ? 'text-green-400' : 'text-red-400'}">
