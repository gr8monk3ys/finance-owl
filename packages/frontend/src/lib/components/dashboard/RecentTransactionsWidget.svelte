...
<Card class="h-full">
    ...
    <div class="mt-2 divide-y divide-surface-700">
      {#each recentTx as tx}
        <div class="flex items-center justify-between py-2">
              ...
              {#if tx.categoryName}
                <span class="inline-flex items-center gap-1 text-surface-300">
                  <span
                    ...
                    style="background-color: {tx.categoryColor || '#60a5fa'}"
                  ></span>
                  {tx.categoryName}
                </span>
              {/if}
          ...
          <span
            class="ml-2 text-xs font-semibold {tx.amount < 0 ? 'text-green-400' : 'text-white'}"
          >
            {fmt(tx.amount)}
          </span>
        </div>
  ...
  {:else}
    <div class="flex flex-col items-center justify-center py-6 text-center">
      ...
      <p class="mt-2 text-xs text-surface-400">No transactions yet</p>
    </div>
  {/if}
</Card>
