<script lang="ts">
	...
	const arrowClasses: Record<string, string> = {
		...
		bottom: 'bottom-full left-1/2 -translate-x-1/2 border-b-surface-700 border-l-transparent border-r-transparent border-t-transparent',
		...
		right: 'right-full top-1/2 -translate-y-1/2 border-r-surface-700 border-t-transparent border-b-transparent border-l-transparent'
	};
</script>
	...
	{#if visible && content}
		...
		>
			...
			>
				...
				<!-- Arrow -->
				<div
					class="absolute h-0 w-0 border-[5px] {arrowClasses[position]}"
				></div>
			</div>
		</div>
	{/if}
