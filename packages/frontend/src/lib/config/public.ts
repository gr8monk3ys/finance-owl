...
const siteUrl = normalizeUrl(readValue(env.PUBLIC_SITE_URL, 'http://localhost:3000'), 'http://localhost:3000');

export const publicSite = Object.freeze({
	appName: readValue(env.PUBLIC_COMPANY_NAME, 'Finance Owl'),
	legalEntityName: readValue(env.PUBLIC_LEGAL_ENTITY_NAME, readValue(env.PUBLIC_COMPANY_NAME, 'Finance Owl')),
