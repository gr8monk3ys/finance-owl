...
<div class="flex min-h-screen bg-surface-900">
  ...
  >
      ...
      {#each groups as group}
        ...
        {#each navItems.filter((i) => i.group === group) as item}
          ...
          >
            ...
            {#if active}
              <div
                class="absolute left-0 top-1/2 -translate-y-1/2 h-6 w-0.5 rounded-full bg-primary-400 nav-indicator"
              ></div>
            {/if}
    ...
    <div
      class="absolute bottom-0 left-0 right-0 border-t border-surface-700/50 p-3 bg-surface-800/50"
    >
      <div
        class="flex items-center gap-3 rounded-lg px-2 py-2 hover:bg-surface-700/40 transition-colors duration-150"
      >
        <div
  ...
  <div class="flex flex-1 flex-col min-w-0">
    <!-- Top bar -->
    <header
      class="sticky top-0 z-30 flex h-16 items-center gap-4 border-b border-surface-700/50 px-4 lg:px-6
    ...
    >
      ...
      <div class="flex items-center gap-2">
        ...
        >
          ...
          <!-- Notification dot -->
          <span
            class="absolute top-1.5 right-1.5 h-2 w-2 rounded-full bg-primary-400 ring-2 ring-surface-800"
          ></span>
        </a>
