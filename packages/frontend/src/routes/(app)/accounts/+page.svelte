<script lang="ts">
  ...
  async function handlePlaidSuccess(publicToken: string) {
    ...
    if (res.ok) {
      ...
      const itemId = result?.data?.linkedPlaidItemId ?? (typeof result === 'string' ? null : null);
      if (itemId) {
        triggerSync(itemId);
      }
    } else {
  ...
  function groupByInstitution(accounts: any[]): { institution: string; accounts: any[] }[] {
...
<div class="space-y-6">
  ...
  {#if syncResult}
    <div
      class="flex items-center justify-between rounded-lg bg-green-900/30 p-3 text-sm text-green-300"
    >
      <span>
        Sync complete: {syncResult.added} added, {syncResult.modified} modified, {syncResult.removed}
        removed
      </span>
  ...
  <details class="rounded-lg border border-surface-700 bg-surface-800/50">
    ...
    <div class="space-y-3 border-t border-surface-700 px-6 py-4">
      <p class="text-xs text-surface-500">
        These tools are only available when the backend is running with PLAID_ENV=sandbox. They
        allow testing the full Plaid flow without the Link UI.
      </p>
