...
describe('accounts page server', () => {
	...
	it('loads accounts, plaid items, and net worth data', async () => {
		global.fetch = vi
			...
			.mockResolvedValueOnce(new Response(JSON.stringify([{ id: 'plaid_1' }]), { status: 200, headers: { 'content-type': 'application/json' } }))
			.mockResolvedValueOnce(
				new Response(JSON.stringify({ assets: 7500, liabilities: 1000, netWorth: 6500, accountCount: 2 }), {
					status: 200,
	...
	it('validates required account-linking inputs', async () => {
		...
		expect(createManualResult).toMatchObject({ status: 400, data: { error: 'Name and type are required' } });
	...
	it('executes the primary account actions through the API', async () => {
		global.fetch = vi
			.fn()
			.mockResolvedValueOnce(new Response(JSON.stringify({ linkToken: 'link-token' }), { status: 200, headers: { 'content-type': 'application/json' } }))
			.mockResolvedValueOnce(
			...
			.mockResolvedValueOnce(new Response(JSON.stringify({ imported: 12 }), { status: 200, headers: { 'content-type': 'application/json' } }))
			.mockResolvedValueOnce(new Response(JSON.stringify({ ok: true }), { status: 200, headers: { 'content-type': 'application/json' } }))
			.mockResolvedValueOnce(new Response(JSON.stringify({ ok: true }), { status: 200, headers: { 'content-type': 'application/json' } }))
			.mockResolvedValueOnce(new Response(JSON.stringify({ linkToken: 'update-token' }), { status: 200, headers: { 'content-type': 'application/json' } }))
			.mockResolvedValueOnce(new Response(JSON.stringify({ ok: true }), { status: 200, headers: { 'content-type': 'application/json' } }))
			.mockResolvedValueOnce(new Response(JSON.stringify({ publicToken: 'sandbox-token' }), { status: 200, headers: { 'content-type': 'application/json' } })) as typeof fetch;
		...
		expect(updateLinkResult).toEqual({ updateLinkToken: 'update-token', updatePlaidItemId: 'plaid_1' });
