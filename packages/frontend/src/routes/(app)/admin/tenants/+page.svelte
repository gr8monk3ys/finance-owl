      ...
      <div class="flex gap-4">
        {#each data.stats.planBreakdown as item}
          <div class="flex items-center gap-2">
            <span
              class="inline-flex items-center rounded-md border px-2 py-0.5 text-xs font-medium capitalize {getPlanColor(
                item.plan,
              )}"
            >
              {item.plan}
    ...
    {:else}
      <div class="divide-y divide-surface-700/50">
        {#each data.tenants as tenant}
          <div
            class="flex items-center gap-4 px-4 py-3 hover:bg-surface-700/20 transition-colors duration-150"
          >
            <!-- Logo / Initial -->
            <div
              class="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg border border-surface-600/50 bg-surface-700/50"
            >
              {#if tenant.logoUrl}
                <img
                  src={tenant.logoUrl}
                ...
                />
              {:else}
                <span class="text-sm font-bold text-surface-300"
            ...
            <div class="min-w-0 flex-1">
              <div class="flex items-center gap-2">
                ...
                <span
                  class="inline-flex items-center rounded-md border px-1.5 py-0.5 text-[10px] font-medium capitalize {getStatusColor(
                    tenant.status,
                  )}"
                >
                  {tenant.status}
                ...
                <span
                  class="inline-flex items-center rounded-md border px-1.5 py-0.5 text-[10px] font-medium capitalize {getPlanColor(
                    tenant.plan,
                  )}"
                >
                  {tenant.plan}
...
{#if showCreateModal}
  <div class="fixed inset-0 z-50 flex items-center justify-center">
    ...
    ></button>
    <div
      class="relative z-10 w-full max-w-md rounded-xl border border-surface-700/50 bg-surface-800 p-6 shadow-xl"
    >
