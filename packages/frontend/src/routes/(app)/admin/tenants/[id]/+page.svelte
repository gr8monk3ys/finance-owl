...
<div class="space-y-6">
	...
	<div class="flex items-center gap-4">
		...
		<span class="ml-auto inline-flex items-center rounded-md border px-2 py-1 text-xs font-medium capitalize
	...
	<div class="grid gap-6 lg:grid-cols-2">
		...
		<div class="rounded-xl border border-surface-700/50 bg-surface-800/50 p-5">
			...
			<div class="space-y-4">
				<div>
					<label for="tenant-name" class="block text-sm font-medium text-surface-300 mb-1.5">Tenant Name</label>
					<input
						id="tenant-name"
						type="text"
				...
				<div>
					<label for="slug" class="block text-sm font-medium text-surface-300 mb-1.5">Slug (read-only)</label>
					<input
						id="slug"
						type="text"
				...
				<div>
					<label for="custom-domain" class="block text-sm font-medium text-surface-300 mb-1.5">Custom Domain</label>
					<input
						id="custom-domain"
						type="text"
				...
				<div class="grid grid-cols-2 gap-4">
					...
					<div>
						<label for="max-users" class="block text-sm font-medium text-surface-300 mb-1.5">Max Users</label>
						<input
							id="max-users"
							type="number"
							bind:value={maxUsers}
		...
		<div class="rounded-xl border border-surface-700/50 bg-surface-800/50 p-5">
			...
			<div class="space-y-4">
				<div>
					<label for="app-name" class="block text-sm font-medium text-surface-300 mb-1.5">App Name</label>
					<input
						id="app-name"
						type="text"
						bind:value={appName}
				...
				<div class="grid grid-cols-2 gap-4">
					<div>
						<label for="primary-color" class="block text-sm font-medium text-surface-300 mb-1.5">Primary Color</label>
					...
					<div>
						<label for="accent-color" class="block text-sm font-medium text-surface-300 mb-1.5">Accent Color</label>
						<div class="flex items-center gap-2">
							<input
								id="accent-color"
								type="color"
				...
				<div>
					<label for="logo-url" class="block text-sm font-medium text-surface-300 mb-1.5">Logo URL</label>
					<input
						id="logo-url"
						type="url"
						bind:value={logoUrl}
				...
				<div>
					<label for="favicon-url" class="block text-sm font-medium text-surface-300 mb-1.5">Favicon URL</label>
					<input
						id="favicon-url"
						type="url"
			...
			<div class="divide-y divide-surface-700/50">
				{#each data.members as member}
					<div class="flex items-center gap-4 px-4 py-3">
						<div class="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-surface-700/50 text-sm font-medium text-surface-300">
						...
						<span class="inline-flex items-center rounded-md border px-2 py-0.5 text-[10px] font-medium capitalize {getRoleColor(member.role)}">
							{member.role}
						</span>
					</div>
				{/each}
