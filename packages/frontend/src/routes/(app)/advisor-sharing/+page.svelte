<script lang="ts">
  ...
  const permissionOptions = [
    ...
    { key: 'budgets', label: 'Budgets', description: 'View budget categories and spending' },
    {
      key: 'investments',
      label: 'Investments',
      description: 'View investment holdings and performance',
    },
          ...
          <div class="rounded-lg border border-surface-700 bg-surface-800 p-4">
            ...
            <div class="mt-3 flex flex-wrap gap-1.5">
              {#each share.permissions as permission}
                <span
                  class="rounded-full px-2 py-0.5 text-xs font-medium capitalize {getPermissionColor(
                    permission,
                  )}"
                >
                  {permission}
    ...
    <div>
      ...
      <div class="mt-3 space-y-2">
        {#each permissionOptions as option}
          <label
            class="flex cursor-pointer items-start gap-3 rounded-lg border border-surface-700 p-3 transition hover:border-surface-600 hover:bg-surface-800"
          >
            <input
              type="checkbox"
...
<!-- Access Logs Modal -->
<Modal open={viewingLogs !== null} onclose={() => (viewingLogs = null)} title="Access Logs">
  {#if viewingLogs}
    <div class="space-y-3">
