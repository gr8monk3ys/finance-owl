...
<div class="space-y-6">
  <div class="flex items-center justify-between">
    ...
    <div class="flex items-center gap-3">
      {#if isAvailable}
        <span
          class="flex items-center gap-2 rounded-full border border-green-500/25 bg-green-500/10 px-3 py-1.5 text-sm text-green-400"
        >
      ...
      {:else}
        <span
          class="flex items-center gap-2 rounded-full border border-surface-600 bg-surface-700/50 px-3 py-1.5 text-sm text-surface-400"
        >
  ...
  {#if activeTab === 'chat'}
    <div class="flex flex-col" style="height: calc(100vh - 18rem);">
          ...
          {#if messages.length === 0}
            <div class="flex h-full flex-col items-center justify-center text-center px-4">
              {#if !isAvailable}
                ...
                <div
                  class="mt-4 w-full max-w-sm rounded-lg border border-surface-700 bg-surface-800/70 p-4 text-left"
                >
                  <p class="text-xs font-medium uppercase tracking-wider text-surface-500 mb-2">
                    To enable AI features:
                  </p>
                  <code
                    class="block rounded bg-surface-900 px-3 py-2 text-sm text-primary-400 font-mono"
                    >docker compose up ollama</code
                  >
                  <p class="mt-2 text-xs text-surface-500">
                    Other features like budgets, transactions, and anomaly detection work without
                    AI.
                  </p>
                </div>
              {:else}
      ...
      >
        ...
        <Button
          type="submit"
          disabled={!isAvailable || isAsking || !question.trim()}
          loading={isAsking}
        >
          <svg
        ...
        >
          ...
          <div>
            ...
            <p class="mt-0.5 text-xs text-yellow-400/70">
              Run <code class="rounded bg-yellow-900/40 px-1 py-0.5 font-mono"
                >docker compose up ollama</code
              > to enable AI-generated analysis. Basic weekly summaries are still generated without AI.
            </p>
          </div>
        </div>
      {/if}
        ...
        <Card>
          <div class="flex flex-col items-center justify-center py-12 text-center">
            ...
            <p class="mt-1 text-sm text-surface-500">
              ...
              {:else}
                Insights will appear here once generated. Enable Ollama for richer AI-powered
                analysis, or check back Monday for basic weekly summaries.
              {/if}
            </p>
          </div>
        </Card>
      {:else}
        {#each data.insights as insight}
          <Card>
            ...
            {#if insight.data}
              <div
                class="mt-4 grid grid-cols-3 gap-4 rounded-lg border border-surface-700 bg-surface-900/50 p-4"
              >
                <div>
