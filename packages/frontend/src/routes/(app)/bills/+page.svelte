<script lang="ts">
	...
	const billsDueThisWeek = $derived(
		data.upcoming.filter(
			(b: any) => b.expectedDate >= todayStr && b.expectedDate <= weekEndStr
		)
	);
		...
		<div class="flex items-center gap-2">
			<div class="flex rounded-lg bg-surface-800 p-0.5">
				{#each [
					...
					{ id: 'list' as const, label: 'List', icon: 'M8.25 6.75h12M8.25 12h12m-12 5.25h12M3.75 6.75h.007v.008H3.75V6.75zm.375 0a.375.375 0 11-.75 0 .375.375 0 01.75 0zM3.75 12h.007v.008H3.75V12zm.375 0a.375.375 0 11-.75 0 .375.375 0 01.75 0zm-.375 5.25h.007v.008H3.75v-.008zm.375 0a.375.375 0 11-.75 0 .375.375 0 01.75 0z' },
					{ id: 'calendar' as const, label: 'Calendar', icon: 'M6.75 3v2.25M17.25 3v2.25M3 18.75V7.5a2.25 2.25 0 012.25-2.25h13.5A2.25 2.25 0 0121 7.5v11.25m-18 0A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75m-18 0v-7.5A2.25 2.25 0 015.25 9h13.5A2.25 2.25 0 0121 11.25v7.5' }
				] as mode}
					<button
						type="button"
	...
	{#if reminderBills.length > 0}
		<Card>
			<div class="flex items-center gap-3">
				<div class="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-lg bg-amber-600/20">
				...
				<div class="flex-1">
					<p class="font-medium text-amber-400">
						{reminderBills.length} bill{reminderBills.length !== 1 ? 's' : ''} due within {reminderDays} days
					</p>
							...
							<div class="flex items-center gap-3">
								...
								<div>
									...
									<p class="text-xs text-red-400">
										{Math.abs(daysUntil(bill.expectedDate))} days overdue -
										was due {formatDate(bill.expectedDate)}
									</p>
								</div>
							</div>
							<div class="flex items-center gap-3">
							...
							<div class="relative border-l-2 border-surface-700 pl-6">
								{#each week.bills as bill, bi}
									...
									<div class="relative mb-4 last:mb-0">
										<!-- Timeline dot -->
										<div class="absolute -left-[31px] top-1 h-4 w-4 rounded-full border-2 {days === 0 ? 'border-amber-400 bg-amber-400' : days <= 1 ? 'border-amber-400 bg-surface-900' : 'border-surface-500 bg-surface-900'}"></div>

										<div class="flex items-center justify-between rounded-lg bg-surface-800/50 p-3">
											<div class="flex items-center gap-3">
												{#if bill.categoryColor}
													<span class="h-2.5 w-2.5 rounded-full" style="background-color: {bill.categoryColor}"></span>
												{/if}
												<div>
													<p class="text-sm font-medium text-white">
														{bill.merchantName || bill.name}
						...
						<div class="flex items-center justify-between py-3">
							<div class="flex items-center gap-3">
								{#if sub.categoryColor}
									<span class="h-2.5 w-2.5 rounded-full" style="background-color: {sub.categoryColor}"></span>
								{/if}
								<div>
									<p class="text-sm font-medium text-white">
							...
							<span class="text-sm font-semibold text-white">
								...
								<span class="text-xs text-surface-500">
									/{sub.frequency === 'annual' ? 'yr' : sub.frequency === 'monthly' ? 'mo' : sub.frequency}
								</span>
							</span>
						</div>
					{/each}
