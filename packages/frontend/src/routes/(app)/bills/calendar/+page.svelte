...
<div class="space-y-6">
  ...
  <div class="flex items-center justify-between">
    ...
    <div class="flex items-center gap-2">
      <a href="/bills" class="text-sm text-primary-400 hover:text-primary-300"> Back to Bills </a>
    </div>
  </div>
  ...
  <div class="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
    ...
    <Card>
      ...
      <p class="mt-0.5 text-xs text-surface-500">Still due this month</p>
    </Card>
    <Card>
      <p class="text-xs font-medium uppercase text-surface-400">Overdue</p>
      <p
        class="mt-1 text-xl font-bold {data.summary.overdueCount > 0
          ? 'text-red-400'
          : 'text-surface-500'}"
      >
      ...
      {#each displayDays as calDay}
        ...
        >
          ...
          {#if calDay.totalDue > 0}
            <div
              class="absolute bottom-1 right-1 rounded bg-surface-700/80 px-1 text-[9px] font-medium text-surface-400 opacity-0 transition group-hover:opacity-100"
            >
              {fmt(calDay.totalDue)}
            </div>
          {/if}
...
<Modal
  ...
  title={selectedDate ? formatFullDate(selectedDate) : 'Day Detail'}
>
          ...
          <div class="ml-3 flex items-center gap-2">
            ...
            {#if !bill.isPaid}
              <form method="POST" action="?/markPaid" use:enhance>
                ...
                <Button type="submit" size="sm" variant="primary">Mark Paid</Button>
              </form>
            {/if}
          </div>
        </div>
