<script lang="ts">
  ...
  const filteredProviders = $derived(
    selectedBillType ? allProviders.filter((p) => p.category === selectedBillType) : allProviders,
  );
  ...
  const succeededAttempts = $derived(attempts.filter((a: any) => a.status === 'succeeded'));
...
<div class="space-y-6">
  ...
  <div class="flex items-center justify-between">
    ...
    <div class="flex items-center gap-3">
      ...
      <a href="/bills" class="text-sm text-surface-400 hover:text-surface-300"> Bills Calendar </a>
    </div>
  </div>
      ...
      <div class="flex items-center gap-3">
        ...
        <div>
          ...
          <p class="text-xs text-surface-500">
            {savings.successfulAttempts} of {savings.totalAttempts}
          </p>
        </div>
      </div>
    </Card>
      ...
      <div class="flex items-center gap-3">
        ...
        <div>
          ...
          <p class="text-xl font-bold text-white">
            {Object.keys(providers).length > 0 ? allProviders.length : 0}
          </p>
  ...
  <Card>
    <div class="flex items-center justify-between">
      {#each [{ step: 1, label: 'Select Bill' }, { step: 2, label: 'Set Target' }, { step: 3, label: 'View Strategy' }, { step: 4, label: 'Track Outcome' }] as item}
        ...
        >
          <span
            class="flex h-6 w-6 items-center justify-center rounded-full text-xs font-bold {wizardStep ===
            item.step
  ...
  {#if wizardStep === 1}
    <Card>
      ...
      <div class="mb-6">
        ...
        <div class="grid grid-cols-2 gap-3 sm:grid-cols-4">
          {#each billTypes as bt}
            <button
              type="button"
              class="flex flex-col items-center gap-2 rounded-lg border p-4 transition {selectedBillType ===
              bt.value
          ...
          {#if filteredProviders.length > 0}
            <div class="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
              {#each filteredProviders as p}
                <button
                  type="button"
                  class="flex items-center justify-between rounded-lg border p-4 text-left transition {selectedProvider ===
                  p.name
      ...
      <div class="flex justify-end pt-4">
        <Button disabled={!selectedBillType || !selectedProvider} onclick={() => goToStep(2)}>
  ...
  {#if wizardStep === 2}
    <Card>
          ...
          <div class="flex items-center justify-between">
            <div>
              ...
              <p class="text-sm text-green-400/70">
                {fmt(currentAmount - targetAmount)}/month ({Math.round(
                  ((currentAmount - targetAmount) / currentAmount) * 100,
                )}% reduction)
              </p>
            </div>
      ...
      <div class="mt-6">
        <span class="mb-2 block text-sm font-medium text-surface-300"
          >Preferred Negotiation Method</span
        >
        <div class="flex gap-3">
          {#each ['phone', 'email', 'chat'] as const as method}
            <button
              type="button"
              class="flex flex-1 items-center justify-center gap-2 rounded-lg border p-3 transition {selectedMethod ===
              method
                ? 'border-primary-500 bg-primary-500/10 text-primary-400'
        ...
        <div class="flex items-start justify-between">
          <div>
            ...
            <div class="mt-2 flex flex-wrap items-center gap-2">
              <span
                class="inline-flex rounded-full bg-primary-500/20 px-2.5 py-1 text-xs font-medium text-primary-400"
              >
                {categoryLabel(strategy.billType)}
              </span>
              <span
                class="inline-flex rounded-full px-2.5 py-1 text-xs font-medium {difficultyBadge(
                  strategy.difficulty,
                )}"
              >
              ...
              <span
                class="inline-flex rounded-full bg-green-500/20 px-2.5 py-1 text-xs font-medium text-green-400"
              >
                {strategy.estimatedSuccessRate}% success rate
              </span>
              <span
                class="inline-flex rounded-full bg-surface-500/20 px-2.5 py-1 text-xs font-medium text-surface-300"
              >
      ...
      <Card>
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div class="flex gap-3">
            ...
            <Button variant="secondary" onclick={() => goToStep(2)}>Adjust Target</Button>
          </div>
          <form
            method="POST"
            action="?/startAttempt"
          ...
          >
            ...
            <Button type="submit">Start Negotiation Attempt</Button>
          </form>
        </div>
      </Card>
  ...
  {#if wizardStep === 4}
    <Card>
      <div class="mb-4 flex items-center justify-between">
        ...
        <Button size="sm" onclick={resetWizard}>New Negotiation</Button>
      </div>
              ...
              <div class="text-right">
                <p class="text-sm">
                  ...
                  <span class="ml-1 font-medium text-green-400"
                    >{fmt(attempt.negotiatedAmount)}</span
                  >
                </p>
    ...
    <Card>
      ...
      <div class="grid gap-4 sm:grid-cols-2">
        <div class="rounded-lg bg-surface-900/50 p-4">
          ...
          <p class="text-sm text-surface-400">
            Call Tuesday through Thursday between 8-10 AM local time. Avoid Mondays (busiest) and
            Fridays (agents want to wrap up). Early morning agents tend to be more helpful.
          </p>
        </div>
        <div class="rounded-lg bg-surface-900/50 p-4">
          <div class="mb-2 flex items-center gap-2">
        ...
        <div class="rounded-lg bg-surface-900/50 p-4">
          ...
          <p class="text-sm text-surface-400">
            If you do not get a good offer, call back and try a different agent. Offers vary
            significantly between representatives. Set a calendar reminder and try again in a few
            days.
          </p>
        </div>
      </div>
