<script lang="ts">
  ...
  const nearLimitCount = $derived(
    data.budgets.filter((b: any) => b.percentUsed >= 80 && b.percentUsed < 100).length,
  );
...
<div class="page-enter space-y-6">
  ...
  <div class="stagger-children grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
    <!-- Total Budget -->
    <div
      class="relative overflow-hidden rounded-xl border border-surface-700/50 bg-gradient-to-br from-surface-800 to-surface-850 p-5"
    >
    ...
    >
      <div
        class="absolute -right-4 -top-4 h-16 w-16 rounded-full blur-xl {data.summary
          .totalRemaining >= 0
          ? 'bg-primary-500/5'
          : 'bg-red-500/5'}"
      ></div>
      <div class="relative">
        <div class="flex items-center gap-2">
          <div
            class="flex h-8 w-8 items-center justify-center rounded-lg {data.summary
              .totalRemaining >= 0
              ? 'bg-primary-500/10'
              : 'bg-red-500/10'}"
          >
        ...
        <p
          class="count-up mt-2 text-2xl font-bold {data.summary.totalRemaining >= 0
            ? 'text-primary-400'
            : 'text-red-400'}"
        >
  ...
  {#if data.budgets.length === 0}
    ...
    >
      <div
        class="absolute inset-0 bg-gradient-to-br from-primary-500/3 via-transparent to-accent-500/3"
      ></div>
            ...
            <div class="flex items-start justify-between gap-4">
              <div class="flex items-center gap-3">
                ...
                <div
                  ...
                  style="background: linear-gradient(135deg, {budget.categoryColor ||
                    '#64748b'}66, {budget.categoryColor || '#64748b'}22)"
                >
                ...
                <div>
                  <div class="flex items-center gap-2">
                    ...
                    <!-- Status badge -->
                    <span
                      class="inline-flex items-center rounded-md px-1.5 py-0.5 text-[10px] font-semibold ring-1 ring-inset {status.class}"
                    >
                      {status.label}
                    </span>
                  </div>
            ...
            <div class="mt-4">
              ...
              <div class="flex items-end justify-between">
                ...
                <div class="text-right">
                  <span
                    class="text-sm font-semibold tabular-nums {getBudgetProgressTextColor(
                      budget.percentUsed,
                    )}"
                  >
...
<Modal open={showCreateModal} onclose={closeCreateModal} title="Create Budget">
  ...
  >
    ...
    <div class="grid grid-cols-2 gap-4">
      <div>
        <label for="budgetAmount" class="block text-sm font-medium text-surface-300">Amount</label>
    ...
    {#if selectedCategoryHasDuplicatePeriod}
      <div
        class="rounded-lg border border-accent-700/40 bg-accent-950/30 px-3 py-2 text-sm text-accent-300"
      >
        A {createPeriod} budget already exists for this category. Choose another period or edit the current
        budget.
      </div>
    {/if}
    ...
    <div class="flex justify-end gap-3 border-t border-surface-700/50 pt-4">
      ...
      <Button
        type="submit"
        disabled={!createCategoryId || !createAmount || selectedCategoryHasDuplicatePeriod}
      >
        Create Budget
      </Button>
    </div>
...
<!-- Edit Budget Modal -->
<Modal open={editingBudget !== null} onclose={() => (editingBudget = null)} title="Edit Budget">
  {#if editingBudget}
    <form
      method="POST"
      action="?/update"
    ...
    >
      ...
      <div>
        <label for="editAmount" class="block text-sm font-medium text-surface-300">Amount</label>
        <div class="relative mt-1">
          <span class="absolute left-3 top-1/2 -translate-y-1/2 text-sm text-surface-500">$</span>
          <input
            id="editAmount"
      ...
      <div class="flex justify-end gap-3 border-t border-surface-700/50 pt-4">
        <Button variant="ghost" type="button" onclick={() => (editingBudget = null)}>Cancel</Button>
    ...
    <form
      ...
      action="?/delete"
      use:enhance
      class="mt-3 border-t border-surface-700/50 pt-3"
    >
      <input type="hidden" name="id" value={editingBudget.id} />
