...
<div class="space-y-6">
  ...
  <div class="flex gap-1 rounded-lg bg-surface-800 p-1">
    {#each tabs as tab}
      ...
      >
        ...
        {#if tab.count > 0}
          <span
            class="ml-1.5 inline-flex h-5 min-w-5 items-center justify-center rounded-full text-xs {activeTab ===
            tab.id
            ...
            <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
              <div class="flex-1 space-y-2">
                ...
                <div class="flex flex-wrap gap-4 text-xs text-surface-500">
                  ...
                  >
                  <span
                    >Median: <span class="text-surface-300">{fmt(suggestion.medianSpending)}</span
                    >/mo</span
                  >
                  <span
                    >Max: <span class="text-surface-300">{fmt(suggestion.maxSpending)}</span
                    >/mo</span
                  >
                </div>
              </div>
              ...
              <div class="flex items-center gap-3">
                <div class="text-right">
                  ...
                  <p class="text-xl font-bold text-primary-400">
                    {fmt(suggestion.suggestedAmount)}
                  </p>
        ...
        {#each data.insights as insight}
          ...
          <div class="rounded-xl border {styles.border} {styles.bg} p-5">
            <div class="flex items-start gap-3">
              <div
                class="flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-full {styles.bg} {styles.color} border {styles.border} text-sm font-bold"
              >
                {styles.icon}
              </div>
              <div class="flex-1">
  ...
  {#if activeTab === 'predictions'}
    ...
    {:else}
      ...
      {#if chartData}
        <Card>
          <h3 class="mb-4 text-sm font-semibold text-surface-300 uppercase tracking-wider">
            Next Month Forecast
          </h3>
              ...
              <div>
                ...
                <div class="mt-1 flex items-center gap-2">
                  ...
                  <span class="{getTrendColor(prediction.trend)} text-sm font-medium">
                    ...
                    {fmtPercent(prediction.monthOverMonthChange)}
                  </span>
                </div>
              </div>
              <span
                class="rounded-full bg-surface-700 px-2 py-0.5 text-xs {getTrendColor(
                  prediction.trend,
                )} capitalize"
              >
                {prediction.trend}
            ...
            <tbody>
              {#each data.patterns as pattern}
                <tr class="border-t border-surface-700">
                  <td class="py-2 pr-4 font-medium text-white whitespace-nowrap"
                    >{pattern.categoryName}</td
                  >
                ...
                <div class="mt-2 flex items-center gap-2 text-xs text-surface-500">
                  <span
                    >Average increase: <span class="text-amber-400 font-medium"
                      >+{pattern.averageIncrease}%</span
                    ></span
                  >
                  <span class="text-surface-600">|</span>
                  <span
                    >Months: {pattern.affectedMonths
                      .map((m: number) => monthNames[m - 1])
                      .join(', ')}</span
                  >
                </div>
              </div>
  ...
  >
    <div class="mx-4 w-full max-w-md rounded-2xl bg-surface-800 p-6 shadow-2xl">
      ...
      <p class="mt-2 text-sm text-surface-400">
        Automatically adjust your budget amounts based on recent spending trends. Choose how
        aggressively budgets should be adjusted.
      </p>
      ...
      >
        <div class="space-y-2">
          ...
          <label
            class="flex cursor-pointer items-start gap-3 rounded-lg border p-3 transition {sensitivity ===
            'conservative'
          ...
          >
            ...
            <div>
              ...
              <p class="text-xs text-surface-400">
                Large adjustments (up to 20%). Closely tracks spending.
              </p>
            </div>
          </label>
        </div>
