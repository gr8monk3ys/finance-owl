...
describe('budgets page server', () => {
	...
	it('loads budgets, categories, and summary data', async () => {
		global.fetch = vi
			.fn()
			.mockResolvedValueOnce(new Response(JSON.stringify([{ id: 'budget_1' }]), { status: 200, headers: { 'content-type': 'application/json' } }))
			.mockResolvedValueOnce(new Response(JSON.stringify([{ id: 'category_1' }]), { status: 200, headers: { 'content-type': 'application/json' } }))
	...
	it('creates, updates, and deletes budgets', async () => {
		...
		const createResult = await actions.create({
			request: createFormRequest({ categoryId: 'groceries', amount: '150', period: 'monthly', rollover: 'on' }),
			locals: { accessToken: 'access-token' }
		} as never);
