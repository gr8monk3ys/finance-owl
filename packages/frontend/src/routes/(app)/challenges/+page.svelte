<script lang="ts">
  ...
  const activeChallenges = $derived(data.challenges.filter((c: any) => c.status === 'active'));
  const pastChallenges = $derived(data.challenges.filter((c: any) => c.status !== 'active'));
</script>

<svelte:head>
  <title>Savings Challenges - Finance Owl</title>
</svelte:head>
...
<div class="space-y-6">
  ...
  <div class="flex items-center justify-between">
    <div>
      ...
      <p class="mt-1 text-sm text-surface-400">Gamify your savings with fun challenges</p>
    </div>
  ...
  {#if activeChallenges.length === 0}
    <Card>
      <div class="flex flex-col items-center justify-center py-12 text-center">
        ...
        <p class="mt-1 text-sm text-surface-500">Start a challenge to begin saving with purpose.</p>
      </div>
    </Card>
  {:else}
    <div class="space-y-3">
      {#each activeChallenges as challenge}
        ...
        <Card>
          ...
          <div class="mt-4 flex gap-2 border-t border-surface-700 pt-3">
            <Button size="sm" onclick={() => (entryChallenge = challenge)}>Log Entry</Button>
