    ...
    <div class="flex items-start gap-3">
      ...
      <div>
        ...
        <p class="mt-0.5 text-xs text-primary-400/70">
          Credit bureau integration is coming soon. For now, you can manually enter your score from
          your bank or credit card provider and use the simulator to explore what-if scenarios.
        </p>
      </div>
    </div>
  </div>
        ...
        <div class="flex flex-col items-center py-4">
          ...
          <div class="relative">
            <svg width="200" height="120" viewBox="0 0 200 120">
              ...
              <!-- Min/Max labels -->
              <text x="10" y="115" fill="#64748b" font-size="11" text-anchor="start">300</text>
              <text x="190" y="115" fill="#64748b" font-size="11" text-anchor="end">850</text>
            </svg>
          </div>

          <p class="mt-2 text-4xl font-bold" style="color: {getScoreColor(data.score.score)}">
            {data.score.score}
          </p>
      ...
      <Card>
        ...
        {:else}
          <div class="flex flex-col items-center justify-center py-12 text-center">
            ...
            <p class="mt-3 text-sm text-surface-400">Add more scores to see your history chart</p>
          </div>
        {/if}
      </Card>
    </div>
      ...
      {:else}
        <Card>
          <p class="py-4 text-center text-sm text-surface-400">
            No factor data available. Add a score with factor details to see the breakdown.
          </p>
        </Card>
      {/if}
    </div>
      ...
      <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <Card>
          <div class="text-center">
            ...
            <p class="mt-1 text-2xl font-bold text-white">
              {data.report.summary?.totalAccounts ?? 0}
            </p>
            <p class="mt-0.5 text-xs text-surface-500">
              {data.report.summary?.openAccounts ?? 0} open / {data.report.summary
                ?.closedAccounts ?? 0} closed
            </p>
          </div>
        </Card>
        <Card>
          <div class="text-center">
            ...
            <p
              class="mt-1 text-2xl font-bold {(data.report.summary?.utilization ?? 0) <= 30
                ? 'text-green-400'
                : (data.report.summary?.utilization ?? 0) <= 50
                  ? 'text-yellow-400'
                  : 'text-red-400'}"
            >
        ...
        <Card>
          <div class="text-center">
            ...
            <p class="mt-1 text-2xl font-bold text-white">
              {data.report.summary?.hardInquiriesLast12Months ?? 0}
            </p>
        ...
        <Card>
          <div class="text-center">
            ...
            <p class="mt-1 text-2xl font-bold text-white">
              {data.report.summary?.oldestAccountAge ?? 'N/A'}
            </p>
            ...
            <div class="divide-y divide-surface-700">
              {#each data.report.accounts.slice(0, 5) as account}
                <div class="flex items-center justify-between px-6 py-3">
                  ...
                  <span
                    class="ml-3 rounded-full border px-2 py-0.5 text-xs font-medium
										{account.status === 'open'
                      ? 'text-green-400 bg-green-400/10 border-green-400/30'
                      : account.status === 'closed'
                        ? 'text-surface-400 bg-surface-400/10 border-surface-400/30'
    ...
    {:else}
      <Card>
        <div class="flex flex-col items-center justify-center py-8 text-center">
          ...
          <p class="mt-1 text-xs text-surface-500">
            If you find inaccurate information on your credit report, file a dispute to get it
            corrected.
          </p>
        </div>
      </Card>
          ...
          <div class="flex items-start gap-3">
            ...
            >
              <svg
                ...
                stroke-width="1.5"
              >
                <path stroke-linecap="round" stroke-linejoin="round" d={scenario.icon} />
              </svg>
            </div>
            <div>
...
<Modal open={showDispute} onclose={() => (showDispute = false)} title="File a Credit Dispute">
  ...
  >
    <div class="space-y-4">
      ...
      <div>
        <label for="explanation" class="block text-sm font-medium text-surface-300"
          >Explanation</label
        >
        <textarea
          id="explanation"
        ...
        >
          ...
          {#if scenarioInfo?.hasParams && (selectedScenario === 'pay_down_debt' || selectedScenario === 'increase_credit_limit')}
            <div class="space-y-3 rounded-lg border border-surface-700 p-4">
              <div>
                <label for="currentUtilization" class="block text-sm font-medium text-surface-300">
                  Current Utilization (%)
                </label>
                <input
                  type="range"
              ...
              <div>
                <label for="targetUtilization" class="block text-sm font-medium text-surface-300">
                  Target Utilization (%)
                </label>
                <input
                  type="range"
          ...
          <div class="mt-4 flex justify-end gap-3">
            <Button variant="secondary" type="button" onclick={closeSimulator}>Cancel</Button>
      ...
      {:else}
        ...
        <div class="space-y-4">
          ...
          <div class="flex items-center justify-center">
            ...
            >
              {simulationResult.scoreChange > 0 ? '+' : ''}{simulationResult.scoreChange} points
            </span>
          </div>
