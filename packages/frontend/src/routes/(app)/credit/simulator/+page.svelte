<script lang="ts">
	...
	function getScenarioIcon(type: string): string {
		const icons: Record<string, string> = {
			...
			open_card: 'M2.25 8.25h19.5M2.25 9h19.5m-16.5 5.25h6m-6 2.25h3m-3.75 3h15a2.25 2.25 0 002.25-2.25V6.75A2.25 2.25 0 0019.5 4.5h-15a2.25 2.25 0 00-2.25 2.25v10.5A2.25 2.25 0 004.5 19.5z',
			...
			increase_limit: 'M2.25 18L9 11.25l4.306 4.307a11.95 11.95 0 015.814-5.519l2.74-1.22m0 0l-5.94-2.28m5.94 2.28l-2.28 5.941'
		};
		return icons[type] ?? '';
	}
	...
	const scorePercent = $derived(
		data.profile ? ((data.profile.currentScore - 300) / 550) * 100 : 0
	);
</script>
			...
			<div class="flex flex-col items-center py-6">
				...
				<div class="mt-4 w-full max-w-md">
					<div class="h-3 overflow-hidden rounded-full bg-surface-700">
						<div
							class="{getScoreBgColor(data.profile.currentScore)} h-full rounded-full transition-all"
		...
		{#if form?.simulationResult}
			<Card>
				<div class="flex items-center gap-4">
					<div class="flex h-12 w-12 items-center justify-center rounded-full {form.simulationResult.estimatedImpact >= 0 ? 'bg-green-900/50' : 'bg-red-900/50'}">
						<span class="text-lg font-bold {getImpactColor(form.simulationResult.estimatedImpact)}">
							{form.simulationResult.estimatedImpact > 0 ? '+' : ''}{form.simulationResult.estimatedImpact}
						</span>
					</div>
					<div class="flex-1">
		...
		<div>
			...
			<div class="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
				{#each scenarios as scenario}
					<button
						class="rounded-lg border border-surface-700 bg-surface-800 p-4 text-left transition-colors hover:border-primary-600 hover:bg-surface-700 {activeScenario === scenario ? 'border-primary-500 ring-1 ring-primary-500' : ''}"
						onclick={() => {
					...
					>
						<div class="flex items-center gap-3">
							<div class="flex h-10 w-10 items-center justify-center rounded-lg bg-surface-700">
								<svg class="h-5 w-5 text-primary-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
									<path stroke-linecap="round" stroke-linejoin="round" d={getScenarioIcon(scenario)} />
								</svg>
							</div>
			...
			<Card padding="none">
				<div class="p-6">
					...
					<p class="mt-1 text-sm text-surface-400">
						Your past what-if scenario results.
					</p>
				</div>
