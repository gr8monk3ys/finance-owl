...
<div class="space-y-6">
  ...
  {#if portfolio}
    <div class="grid grid-cols-2 gap-4 lg:grid-cols-4">
      <Card>
        <p class="text-xs font-medium uppercase tracking-wider text-surface-400">Portfolio Value</p>
      ...
      <Card>
        <p class="text-xs font-medium uppercase tracking-wider text-surface-400">Total Gain/Loss</p>
  ...
  <div class="border-b border-surface-700">
    <nav class="-mb-px flex gap-6">
      {#each [{ id: 'holdings', label: 'Holdings' }, { id: 'transactions', label: 'Transactions' }, { id: 'watchlist', label: 'Watchlist' }] as tab}
        <button
        ...
        >
          {tab.label}
          {#if tab.id === 'holdings'}
            <span class="ml-1.5 rounded-full bg-surface-700 px-2 py-0.5 text-xs text-surface-300">
      ...
      <div class="hidden overflow-x-auto lg:block">
        <Card padding="none">
          <table class="w-full">
            <thead>
              <tr
                class="border-b border-surface-700 text-left text-xs font-medium uppercase tracking-wider text-surface-400"
              >
                <th class="px-6 py-3">
        ...
        {#each sortedHoldings() as holding}
          ...
          <Card>
            ...
            <div class="mt-3 flex gap-2 border-t border-surface-700 pt-3">
              <Button size="sm" variant="ghost" onclick={() => openRecordTx(holding.id)}>
                Transaction
              </Button>
              <Button size="sm" variant="ghost" onclick={() => openEditHolding(holding)}>
                Edit
              </Button>
              <Button
                size="sm"
    ...
    {:else}
      <Card>
        <div class="flex flex-col items-center justify-center py-12 text-center">
          ...
          <div class="mt-3">
            <Button size="sm" onclick={() => openRecordTx()}>Record First Transaction</Button>
          </div>
        </div>
      </Card>
    {/if}
    ...
    {:else}
      <Card>
        <div class="flex flex-col items-center justify-center py-12 text-center">
          ...
          <div class="mt-3">
            <Button size="sm" onclick={() => (showAddWatchlist = true)}>Add First Coin</Button>
          </div>
        </div>
      </Card>
    {/if}
...
<Modal open={showDeleteConfirm} onclose={() => (showDeleteConfirm = false)} title="Delete Holding">
  <form method="POST" action="?/deleteHolding" use:enhance>
    ...
    <div class="mt-6 flex justify-end gap-3">
      <Button variant="secondary" type="button" onclick={() => (showDeleteConfirm = false)}>
        Cancel
      </Button>
...
<Modal open={showRecordTx} onclose={() => (showRecordTx = false)} title="Record Transaction">
...
>
  <div class="space-y-4">
    ...
    <div class="flex gap-2">
      {#each [{ days: 7, label: '7D' }, { days: 30, label: '30D' }, { days: 90, label: '90D' }, { days: 365, label: '1Y' }] as period}
        <button
          class="rounded-lg px-3 py-1.5 text-sm font-medium transition {chartPeriod === period.days
