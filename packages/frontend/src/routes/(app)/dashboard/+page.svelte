<script lang="ts">
  ...
  function toggleWidget(id: string) {
    editWidgets = editWidgets.map((w) => (w.id === id ? { ...w, visible: !w.visible } : w));
  }
...
<div class="page-enter space-y-8">
  ...
  <div class="stagger-children grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
    ...
    <div
      class="relative overflow-hidden rounded-xl border border-primary-800/40 bg-gradient-to-br from-primary-950 via-surface-800 to-surface-800 p-6 shadow-lg sm:col-span-2"
    >
    ...
    <div
      class="relative overflow-hidden rounded-xl border border-blue-800/30 bg-gradient-to-br from-blue-950/80 via-surface-800 to-surface-800 p-6 shadow-md"
    >
    ...
    <!-- Budget Remaining -->
    <div
      class="relative overflow-hidden rounded-xl border border-surface-700/50 bg-gradient-to-br from-surface-800 to-surface-850 p-6 shadow-md"
    >
      <div class="relative">
        <div class="flex items-center gap-2">
        ...
        {:else}
          ...
          <p class="mt-1 text-xs text-surface-400">
            <a
              href="/budgets"
              class="text-primary-400 hover:text-primary-300 underline decoration-primary-400/30 underline-offset-2"
              >Set up budgets</a
            >
          </p>
        {/if}
  ...
  {#if !hasData}
    <!-- Empty state -->
    <div
      class="relative overflow-hidden rounded-2xl border border-surface-700/50 bg-gradient-to-br from-surface-800 to-surface-850 p-1"
    >
      <div
        class="absolute inset-0 bg-gradient-to-br from-primary-500/5 via-transparent to-blue-500/5"
      ></div>
  ...
  {:else}
    ...
    <div class="stagger-children grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
      {#each visibleWidgets as widget (widget.id)}
        <div
          class="min-h-[280px] {widget.type === 'net-worth' ? 'sm:col-span-2 lg:col-span-1' : ''}"
        >
          {#if widget.type === 'net-worth'}
            <NetWorthWidget netWorth={data.netWorth} netWorthHistory={data.netWorthHistory} />
          {:else if widget.type === 'spending-category'}
            <SpendingCategoryWidget categoryBreakdown={data.dashboard.categoryBreakdown} />
          {:else if widget.type === 'budget-progress'}
            <BudgetProgressWidget budgets={data.budgets} />
          {:else if widget.type === 'recent-transactions'}
            <RecentTransactionsWidget transactions={data.dashboard.recentTransactions} />
  ...
  <div class="space-y-4">
    ...
    <div class="max-h-96 divide-y divide-surface-700/50 overflow-y-auto">
      {#each editWidgets as widget, i (widget.id)}
        <div
          class="flex items-center gap-3 py-3 transition-colors hover:bg-surface-750/50 px-2 -mx-2 rounded-lg"
        >
          <!-- Toggle -->
          <button
            type="button"
