            ...
            <table class="w-full text-sm">
              ...
              <tbody class="divide-y divide-surface-700/50">
                {#each [strategyComparison.avalanche, strategyComparison.snowball, strategyComparison.minimumOnly] as row}
                  <tr class={row.strategy === selectedStrategy ? 'bg-primary-500/10' : ''}>
                    <td class="py-3 font-medium text-white capitalize">
                      {row.strategy === 'minimum_only' ? 'Minimum Only' : row.strategy}
        ...
        {#if payoffPlan}
          ...
          {#if payoffPlan.schedules && payoffPlan.schedules.length > 0}
            <Card>
              ...
              <p class="mt-1 text-sm text-surface-400">When each debt will be paid off</p>
            ...
            <Card>
              ...
              <p class="mt-1 text-sm text-surface-400">Projected balances month by month</p>
            ...
            <Card>
              <h3 class="text-lg font-semibold text-white">Month-by-Month Amortization</h3>
              <p class="mt-1 text-sm text-surface-400">
                ...
                {#each payoffPlan.schedules as schedule, i}
                  <div class="rounded-lg border border-surface-700">
                    ...
                    {#if expandedSchedule === schedule.debtId}
                      <div class="max-h-72 overflow-y-auto border-t border-surface-700 p-3">
                        <table class="w-full text-xs">
                          <thead>
                            <tr class="border-b border-surface-700">
                              <th class="pb-2 text-left text-surface-400">Month</th>
                              <th class="pb-2 text-right text-surface-400"> Payment </th>
                              <th class="pb-2 text-right text-surface-400"> Principal </th>
                              <th class="pb-2 text-right text-surface-400"> Interest </th>
                              <th class="pb-2 text-right text-surface-400"> Balance </th>
                            </tr>
                          </thead>
                          <tbody class="divide-y divide-surface-700/30">
                            {#each schedule.months as entry}
                              <tr>
                                <td class="py-1.5 text-surface-300">{entry.month}</td>
...
<Modal open={editingDebt !== null} onclose={() => (editingDebt = null)} title="Edit Debt">
  {#if editingDebt}
    ...
    >
      ...
      <div class="flex justify-end gap-3 pt-2">
        <Button variant="ghost" type="button" onclick={() => (editingDebt = null)}>Cancel</Button>
...
<Modal open={payingDebt !== null} onclose={() => (payingDebt = null)} title="Record Payment">
  {#if payingDebt}
    ...
    >
      ...
      <div class="flex justify-end gap-3 pt-2">
        <Button variant="ghost" type="button" onclick={() => (payingDebt = null)}>Cancel</Button>
