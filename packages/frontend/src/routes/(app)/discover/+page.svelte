<script lang="ts">
  ...
  const totalUnclaimedAmount = $derived(
    foundResults.reduce((sum: number, r: any) => sum + (r.reportedAmount ?? 0), 0),
  );
</script>
...
<div class="space-y-6">
  ...
  <div class="flex gap-1 rounded-lg bg-surface-800 p-1">
    <button
      class="flex-1 rounded-md px-3 py-1.5 text-sm font-medium transition {activeSection ===
      'unclaimed'
  ...
  {#if activeSection === 'unclaimed'}
    ...
    <Card>
      ...
      <p class="mb-4 text-sm text-surface-400">
        Search state databases for unclaimed property, forgotten bank accounts, insurance payouts,
        and more.
      </p>
      <form
        method="POST"
              ...
              <div class="flex items-start justify-between gap-4">
                ...
                <div class="flex flex-col gap-2">
                  ...
                  <form method="POST" action="?/updateResultStatus" use:enhance>
                    ...
                    <Button variant="secondary" size="sm" type="submit">Mark Claimed</Button>
                  </form>
                  <form method="POST" action="?/updateResultStatus" use:enhance>
                    <input type="hidden" name="id" value={result.id} />
    ...
    <div class="flex items-center justify-between">
      <p class="text-sm text-surface-400">
        {#if filteredRecommendations.length > 0}
          {filteredRecommendations.length} recommendation{filteredRecommendations.length !== 1
            ? 's'
            : ''}
        {:else}
          ...
          <Card>
            <div class="flex items-start gap-4">
              ...
              <div class="flex flex-col items-center">
                <div
                  class="flex h-14 w-14 items-center justify-center rounded-full border-2 {rec.matchScore >=
                  80
                    ? 'border-green-500'
                    : rec.matchScore >= 60
                      ? 'border-yellow-500'
                ...
                <div class="mt-3 flex flex-wrap gap-3 text-sm">
                  ...
                  {#if rec.annualFee !== null && rec.annualFee !== undefined}
                    <span class="text-surface-400">
                      ...
                      {:else}
                        <span class="font-semibold text-surface-300">{fmt(rec.annualFee)}</span> annual
                        fee
                      {/if}
                    </span>
                  {/if}
