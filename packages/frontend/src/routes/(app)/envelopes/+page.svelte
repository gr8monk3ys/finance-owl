...
<svelte:head>
  <title>Envelopes - Finance Owl</title>
</svelte:head>
...
<div class="space-y-6">
  ...
  <div class="rounded-xl border border-surface-700 bg-surface-800 p-5">
    <div
      class="flex flex-col items-center justify-center text-center sm:flex-row sm:justify-between sm:text-left"
    >
      <div>
  ...
  {#if data.envelopes.length > 0}
    <div class="flex justify-end border-t border-surface-700 pt-4">
      <form method="POST" action="?/rollover" use:enhance>
        <Button variant="ghost" type="submit" size="sm">Roll Over to Next Period</Button>
      </form>
    </div>
  {/if}
</div>
