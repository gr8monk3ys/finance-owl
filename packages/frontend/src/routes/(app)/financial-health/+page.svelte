<script lang="ts">
	...
	const gaugeDashOffset = $derived(
		gaugeCircumference - (overallScore / 100) * gaugeCircumference
	);
</script>
	...
	<div class="flex items-center justify-between">
		<div>
			...
			<p class="mt-1 text-sm text-surface-400">
				Your composite financial wellness score
			</p>
		</div>
					...
					<Card>
						...
						<div class="mt-3">
							<div class="h-2 overflow-hidden rounded-full bg-surface-700">
								<div
									class="h-full rounded-full transition-all {goal.isAchieved ? 'bg-green-500' : 'bg-primary-500'}"
...
<Modal open={showGoalModal} onclose={() => (showGoalModal = false)} title="Add Financial Goal">
	...
	>
		...
		<div class="flex justify-end gap-3 pt-2">
			<Button variant="ghost" type="button" onclick={() => (showGoalModal = false)}>
				Cancel
			</Button>
