<script lang="ts">
	...
	const rawFlags = $derived(
		activeTab === 'household' ? data.householdFlags : data.flags
	);

	const displayedFlags = $derived(
		statusFilter === 'all'
			? rawFlags
			: rawFlags.filter((f: any) => f.status === statusFilter)
	);
...
<div class="space-y-6">
		...
		<div class="flex gap-1 rounded-lg bg-surface-800 p-1">
			...
			>
				...
				{#if data.flags.length > 0}
					<span class="ml-1.5 rounded-full bg-surface-600 px-1.5 py-0.5 text-xs">{data.flags.length}</span>
				{/if}
			</button>
			<button
			...
			>
				...
				{#if data.householdFlags.length > 0}
					<span class="ml-1.5 rounded-full bg-surface-600 px-1.5 py-0.5 text-xs">{data.householdFlags.length}</span>
				{/if}
			</button>
		</div>
	{/if}
	...
	<div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
		...
		<div class="flex gap-1 rounded-lg bg-surface-800 p-1">
			{#each [
				...
				<button
					class="rounded-md px-3 py-1.5 text-xs font-medium transition {statusFilter === filter.value
						? 'bg-surface-700 text-white shadow-sm'
						: 'text-surface-400 hover:text-white'}"
	...
	{#if openDisplayed.length > 0}
		<div>
			<h3 class="mb-3 flex items-center gap-2 text-sm font-semibold uppercase tracking-wider text-surface-400">
						...
						<div class="flex items-start justify-between gap-4">
							<div class="min-w-0 flex-1">
								<div class="flex items-center gap-2">
									...
									<span class="rounded-full bg-yellow-500/15 px-2 py-0.5 text-xs font-medium text-yellow-400 border border-yellow-500/25">
										Open
									</span>
								</div>
								<div class="mt-1.5 flex flex-wrap items-center gap-x-4 gap-y-1 text-sm text-surface-400">
									...
									{#if flag.transactionCategory}
										<span class="rounded bg-surface-700 px-1.5 py-0.5 text-xs">{flag.transactionCategory}</span>
									{/if}
								</div>
								{#if flag.reason}
									<div class="mt-2.5 flex items-start gap-2 rounded-lg bg-surface-700/50 px-3 py-2">
										<svg class="mt-0.5 h-4 w-4 flex-shrink-0 text-surface-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
								...
								<p class="mt-2 text-xs text-surface-500">
									Flagged by <span class="text-surface-400">{flag.flaggedByName ?? 'Unknown'}</span> {timeAgo(flag.createdAt)}
								</p>
							</div>
							<Button variant="secondary" size="sm" onclick={() => { resolvingFlag = flag; resolveComment = ''; }}>
								Resolve
							</Button>
						</div>
					</Card>
							...
							<div class="min-w-0 flex-1">
								...
								{#if flag.resolveComment}
									<div class="mt-2 flex items-start gap-2 rounded-lg bg-green-900/15 px-3 py-2">
										<svg class="mt-0.5 h-4 w-4 flex-shrink-0 text-green-500/50" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
...
<Modal open={showFlagModal} onclose={() => (showFlagModal = false)} title="Flag a Transaction">
	...
	>
		...
		<div class="flex justify-end gap-3 pt-2">
			<Button variant="ghost" type="button" onclick={() => (showFlagModal = false)}>
				Cancel
			</Button>
			<Button type="submit">
				<svg class="mr-1.5 h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
	...
	{#if resolvingFlag}
		<div class="space-y-4">
			...
			<div class="rounded-lg border border-surface-700 bg-surface-900/50 p-4">
				<p class="font-medium text-white">{resolvingFlag.transactionName ?? 'Unknown Transaction'}</p>
