...
<div class="space-y-6">
  ...
  <div class="flex items-center justify-between">
    <div>
      ...
      <p class="mt-1 text-sm text-surface-400">
        Project your finances forward based on recurring patterns
      </p>
    </div>
    <div class="flex gap-1 rounded-lg bg-surface-800 p-1">
      {#each [3, 6, 12] as months}
        <button
          class="rounded-md px-3 py-1.5 text-sm font-medium transition {data.months === months
  ...
  <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
      ...
      <div class="flex items-center gap-3">
        ...
        <div>
          ...
          <p class="text-xl font-bold text-green-400">
            {fmt(data.cashFlow.monthlyRecurringIncome)}
          </p>
          <p class="text-xs text-surface-500">
            {incomeCount} recurring source{incomeCount !== 1 ? 's' : ''}
          </p>
        </div>
      </div>
    </Card>
      ...
      <div class="flex items-center gap-3">
        ...
        <div>
          ...
          <p class="text-xl font-bold text-red-400">
            {fmt(data.cashFlow.monthlyRecurringExpenses)}
          </p>
          <p class="text-xs text-surface-500">
    ...
    <Card>
      <div class="flex items-center gap-3">
        <div
          class="flex h-10 w-10 items-center justify-center rounded-lg {projectedChange >= 0
            ? 'bg-green-600/20'
            : 'bg-red-600/20'}"
        >
          <svg
        ...
        <div>
          ...
          <p class="text-xl font-bold {projectedEndBalance >= 0 ? 'text-white' : 'text-red-400'}">
  ...
  <Card>
    <div class="flex items-center justify-between">
      <div class="flex items-center gap-4">
        <div
          class="flex h-12 w-12 items-center justify-center rounded-full {data.cashFlow
      ...
      <div class="text-right">
        <p
          class="text-sm font-medium {data.cashFlow.netMonthlyCashFlow >= 0
            ? 'text-green-400'
            : 'text-red-400'}"
        >
  ...
  <Card>
    <div class="mb-4 flex items-center justify-between">
      <div>
        ...
        <p class="text-xs text-surface-500">
          Shaded areas show optimistic/pessimistic scenarios (+-15%)
        </p>
      </div>
      {#if balanceTrend() === 'positive'}
        <span
          class="inline-flex items-center rounded-full bg-green-900/50 px-2.5 py-0.5 text-xs font-medium text-green-400"
        >
          Upward trend
        </span>
      {:else if balanceTrend() === 'negative'}
        <span
          class="inline-flex items-center rounded-full bg-red-900/50 px-2.5 py-0.5 text-xs font-medium text-red-400"
        >
          Downward trend
        </span>
      {/if}
          ...
          <tbody>
            ...
            {#each displayedMonths as month}
              ...
              <tr class="border-b border-surface-700/50 hover:bg-surface-700/30 transition">
                ...
                <td
                  class="px-4 py-2 text-right font-medium {net >= 0
                    ? 'text-green-400'
                    : 'text-red-400'}"
                >
                  {fmt(net)}
  ...
  <div class="grid gap-6 lg:grid-cols-2">
    ...
    <Card>
      ...
      {:else}
        <p class="py-4 text-center text-sm text-surface-400">No recurring expenses found.</p>
      {/if}
    </Card>
  </div>
    ...
    <div class="grid gap-4 sm:grid-cols-2">
      <div class="space-y-3">
        <div class="flex items-start gap-3">
          <span
            class="mt-0.5 flex h-6 w-6 flex-shrink-0 items-center justify-center rounded-full bg-surface-700 text-xs font-medium text-surface-300"
            >1</span
          >
          <div>
            <p class="text-sm font-medium text-surface-300">Recurring patterns are stable</p>
            <p class="text-xs text-surface-500">
              Income and expenses repeat at the same amounts and frequencies each period.
            </p>
          </div>
        </div>
        <div class="flex items-start gap-3">
          ...
          <div>
            ...
            <p class="text-xs text-surface-500">
              The forecast does not account for irregular large purchases, bonuses, or windfalls.
            </p>
          </div>
        </div>
        <div class="flex items-start gap-3">
          ...
          <div>
            ...
            <p class="text-xs text-surface-500">
              Where recurring transactions do not cover a category, budget amounts are used as
              expense estimates.
            </p>
          </div>
        </div>
      ...
      <div class="space-y-3">
        <div class="flex items-start gap-3">
          ...
          <div>
            ...
            <p class="text-xs text-surface-500">
              The optimistic and pessimistic scenarios assume a 15% variance from projected net cash
              flow.
            </p>
          </div>
        </div>
        <div class="flex items-start gap-3">
          ...
          <div>
            ...
            <p class="text-xs text-surface-500">
              Savings and investment growth are not factored into balance projections.
            </p>
          </div>
        </div>
        <div class="flex items-start gap-3">
          ...
          <div>
            <p class="text-sm font-medium text-surface-300">
              Based on {incomeCount + expenseCount} recurring items
            </p>
            <p class="text-xs text-surface-500">
              {incomeCount} income source{incomeCount !== 1 ? 's' : ''} and {expenseCount} expense{expenseCount !==
              1
                ? 's'
