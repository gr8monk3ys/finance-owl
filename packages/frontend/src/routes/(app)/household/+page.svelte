<script lang="ts">
	...
	const unsavedAccounts = $derived(
		data.accounts.filter(
			(a: any) => !data.sharedAccounts.some((sa: any) => sa.accountId === a.id)
		)
	);
</script>
...
<div class="space-y-6">
			...
			<Card>
				<div class="flex flex-col items-center justify-center py-8 text-center">
					...
					<p class="mt-2 text-sm text-surface-400">
						Start a new household and invite family members to share accounts and track spending together.
					</p>
					<div class="mt-6">
	...
	{:else}
		...
		{#if data.households.length > 1}
			<div class="flex items-center gap-3 rounded-lg border border-surface-700 bg-surface-800/50 px-4 py-3">
				...
				<span class="text-sm text-surface-300">
					You belong to {data.households.length} households. Viewing: <strong class="text-white">{data.household.name}</strong>
				</span>
			</div>
		{/if}
							...
							<div>
								...
								<p class="mt-0.5 text-sm text-surface-400">
									...
									{#if data.sharedAccounts.length > 0}
										 -- {data.sharedAccounts.length} shared account{data.sharedAccounts.length !== 1 ? 's' : ''}
									{/if}
								</p>
							</div>
						</div>
						{#if isOwner}
							<span class="rounded-full bg-yellow-500/15 px-2.5 py-1 text-xs font-medium text-yellow-300 border border-yellow-500/25">
								Owner
							</span>
						{/if}
					</div>
					...
					<div class="mt-5 rounded-lg border border-surface-700 bg-surface-900/50 p-4">
						<div class="flex items-center justify-between">
							<div>
								...
								{:else}
									<p class="mt-1.5 text-sm text-surface-500 italic">
							...
							<div class="flex gap-2">
								{#if data.household.inviteCode}
									<Button variant="ghost" size="sm" onclick={copyInviteCode}>
										{#if copiedCode}
											<svg class="mr-1.5 h-4 w-4 text-green-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
					...
					<div class="mt-4 divide-y divide-surface-700">
						{#each data.household.members ?? [] as member}
							<div class="flex items-center justify-between py-3.5">
								...
								<div class="flex items-center gap-3">
									<span
										class="rounded-full px-2.5 py-0.5 text-xs font-medium capitalize {getRoleBadgeColor(member.role)}"
									>
										{member.role}
									</span>
				...
				<Card>
					...
					{#if data.sharedAccounts.length === 0}
						<div class="mt-4 flex flex-col items-center py-4 text-center">
							...
							{#if unsavedAccounts.length > 0}
								<Button variant="secondary" size="sm" class="mt-3" onclick={() => (showShareAccountModal = true)}>
									Share an Account
								</Button>
							{/if}
						</div>
					{:else}
						<div class="mt-4 space-y-2">
							{#each data.sharedAccounts as sa}
								<div class="flex items-center justify-between rounded-lg border border-surface-700 bg-surface-900/50 p-3">
						...
						{:else}
							...
							>
								...
								<div>
									...
									<p class="text-xs text-surface-500">Permanently delete this household and all shared data</p>
								</div>
							</button>
						{/if}
					</div>
...
<Modal open={showJoinModal} onclose={() => (showJoinModal = false)} title="Join Household">
	...
	>
		...
		<div class="flex justify-end gap-3 pt-2">
			<Button variant="ghost" type="button" onclick={() => (showJoinModal = false)}>
				Cancel
			</Button>
	...
	{#if data.household}
		{#if unsavedAccounts.length === 0}
			<div class="flex flex-col items-center py-4 text-center">
				<svg class="h-12 w-12 text-surface-600" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1">
					<path stroke-linecap="round" stroke-linejoin="round" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
				</svg>
				<p class="mt-3 text-sm text-surface-300">All your accounts are already shared.</p>
			</div>
			<div class="mt-4 flex justify-end">
				<Button variant="ghost" onclick={() => (showShareAccountModal = false)}>
					Close
				</Button>
			</div>
		{:else}
			...
			>
				...
				<div class="flex justify-end gap-3 pt-2">
					<Button
						variant="ghost"
						type="button"
						onclick={() => (showShareAccountModal = false)}
					>
						Cancel
					</Button>
	...
	{#if editingMember && data.household}
		<div class="space-y-4">
			...
			>
				...
				<div>
					<label for="memberRole" class="block text-sm font-medium text-surface-300">
						Role
					</label>
					<select
						id="memberRole"
	...
	{#if confirmRemoveMember && data.household}
		<div class="space-y-4">
			<div class="flex items-center gap-3 rounded-lg border border-red-700/30 bg-red-900/20 p-4">
				<svg class="h-6 w-6 flex-shrink-0 text-red-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
		...
		<div class="space-y-4">
			<div class="flex items-center gap-3 rounded-lg border border-red-700/30 bg-red-900/20 p-4">
				...
				<div>
					...
					<p class="mt-1 text-sm text-surface-400">
						You will lose access to all shared accounts and data. You can rejoin later with a new invite code.
					</p>
				</div>
			</div>
		...
		<div class="space-y-4">
			<div class="flex items-center gap-3 rounded-lg border border-red-700/30 bg-red-900/20 p-4">
				...
				<div>
					...
					<p class="mt-1 text-sm text-surface-400">
						All members will be removed and all shared account connections will be dissolved. This action cannot be undone.
					</p>
				</div>
			</div>
