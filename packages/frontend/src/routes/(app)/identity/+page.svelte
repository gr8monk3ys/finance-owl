<script lang="ts">
  ...
  const acknowledgedBreaches = $derived((data.breaches ?? []).filter((b: any) => b.isAcknowledged));
  ...
  <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
    <Card>
      <div class="flex items-center gap-3">
        <div
          class="flex h-10 w-10 items-center justify-center rounded-xl {data.summary.totalBreaches >
          0
            ? 'bg-red-500/15'
            : 'bg-green-500/15'}"
        >
        ...
        <div>
          ...
          <p
            class="text-2xl font-bold {data.summary.totalBreaches > 0
              ? 'text-red-400'
              : 'text-green-400'}"
          >
    ...
    <Card>
      <div class="flex items-center gap-3">
        <div
          class="flex h-10 w-10 items-center justify-center rounded-xl {passwordExposureCount > 0
            ? 'bg-orange-500/15'
            : 'bg-green-500/15'}"
        >
          <svg
        ...
        <div>
          ...
          <p
            class="text-2xl font-bold {passwordExposureCount > 0
              ? 'text-orange-400'
              : 'text-green-400'}"
          >
    ...
    <Card>
      <div class="flex items-center gap-3">
        <div
          class="flex h-10 w-10 items-center justify-center rounded-xl {severityBg(
        ...
        >
          ...
          >
            <path
              stroke-linecap="round"
              stroke-linejoin="round"
              d={severityIcon(data.summary.severity)}
            />
          </svg>
        </div>
        <div>
          ...
          >
            <h3 class="flex items-center gap-2 text-lg font-semibold text-surface-400">
              ...
              <span class="text-sm font-normal text-surface-500"
                >({acknowledgedBreaches.length})</span
              >
            </h3>
            <svg
              ...
              {#each acknowledgedBreaches as breach}
                ...
                <Card>
                  <div class="flex items-start justify-between gap-4">
                    <div class="min-w-0 flex-1">
                      ...
                      {#if dataClasses.length > 0}
                        <div class="mt-2 flex flex-wrap gap-1.5">
                          {#each dataClasses as dc}
                            <span
                              class="rounded bg-surface-700/50 px-2 py-0.5 text-xs text-surface-500"
                            >
                              {dc}
                            </span>
                          {/each}
                    ...
                    <span
                      class="flex-shrink-0 rounded-full bg-surface-700 px-2.5 py-1 text-xs text-surface-400"
                    >
                      Acknowledged
                    </span>
                  </div>
          ...
          <Card padding="none">
            <div class="relative">
              {#each breachTimeline as breach, i}
                ...
                {@const hasCritical = dataClasses.some((dc: string) => isCriticalDataClass(dc))}
                <div
                  class="relative flex gap-4 px-6 py-4 {i < breachTimeline.length - 1
                ...
                >
                  ...
                  <div class="flex flex-col items-center">
                    <div
                      class="flex h-8 w-8 items-center justify-center rounded-full {breach.isAcknowledged
                        ? 'bg-surface-700'
                        : hasCritical
                          ? 'bg-red-500/20'
                          : 'bg-orange-500/20'}"
                    >
                      <span
                        class="h-3 w-3 rounded-full {breach.isAcknowledged
                          ? 'bg-surface-500'
                          : hasCritical
                            ? 'bg-red-400'
                            : 'bg-orange-400'}"
                ...
                <div class="flex items-center justify-between gap-2">
                  <div class="min-w-0 flex-1">
                    ...
                    <div class="mt-1 flex items-center gap-3 text-xs text-surface-500">
                      <span
                        >{monitored.totalBreaches} breach{monitored.totalBreaches !== 1
                          ? 'es'
                          : ''}</span
                      >
        ...
        {:else}
          <Card class="mt-3">
            <div class="py-4 text-center">
              ...
              <p class="mt-3 text-sm text-surface-400">No emails being monitored yet.</p>
              <p class="mt-1 text-xs text-surface-500">
      ...
      <div>
        ...
        <Card>
          <p class="mb-4 text-sm text-surface-400">
            Check if a password has appeared in known data breaches. Your password is hashed locally
            using SHA-1 before checking (k-anonymity model).
          </p>
          ...
          <form
            ...
            onsubmit={async (e) => {
              ...
              const hiddenInput = formEl.querySelector(
                'input[name="sha1Hash"]',
              ) as HTMLInputElement;
              if (hiddenInput) {
            ...
            {:else}
              <div class="mt-4 rounded-lg border border-green-700/30 bg-green-900/20 p-4">
                ...
                <p class="mt-2 text-sm text-green-200/80">
                  This password has not been found in any known data breaches. That said, always use
    ...
    <Card>
      <div class="flex flex-col items-center justify-center py-12 text-center">
        ...
        <p class="mt-1 text-sm text-surface-500 max-w-sm">
          Add an email address to start monitoring for data breaches, or check if a password has
          been exposed.
        </p>
      </div>
    </Card>
