<script lang="ts">
	...
	function getSelectedTransactions(): any[] {
		return previewRows.filter((r) => r.selected).map((r) => ({
			date: r.date,
			name: r.name,
			amount: r.amount,
			category: r.category,
		...
		<Card>
			...
			>
				...
				>
					...
					{:else}
						<svg class="h-12 w-12 text-surface-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
						...
						<p class="mt-1 text-xs text-surface-500">
							Supports .csv, .ofx, .qfx files (max 10MB)
						</p>
					{/if}
				</div>
					...
					>
						...
						{#each data.accounts as account}
							<option value={account.id}>
								{account.name}
								{#if account.institutionName} ({account.institutionName}){/if}
							</option>
						{/each}
					</select>
				</Card>
					...
					<Card>
						<div class="mb-4 flex items-center justify-between">
							...
							{#if detectedFormat && detectedFormat !== 'custom'}
								<span class="rounded-full bg-green-900/50 px-3 py-1 text-xs font-medium text-green-400">
						...
						{#if columnMapping}
							<div class="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
								<div>
									<label for="col-date" class="mb-1 block text-xs text-surface-400">Date Column</label>
									<select
										id="col-date"
										...
										onchange={(e) => updateMapping('date', parseInt((e.target as HTMLSelectElement).value))}
									>
								...
								<div>
									<label for="col-description" class="mb-1 block text-xs text-surface-400">Description Column</label>
									<select
										id="col-description"
										class="w-full rounded-lg border border-surface-600 bg-surface-700 px-3 py-2 text-sm text-white focus:border-primary-500 focus:outline-none"
										value={columnMapping.description}
										onchange={(e) => updateMapping('description', parseInt((e.target as HTMLSelectElement).value))}
									>
										{#each uploadResult.headers as header, i}
											<option value={i}>{header}</option>
										{/each}
								...
								<div>
									<label for="col-amount" class="mb-1 block text-xs text-surface-400">Amount Column</label>
									<select
										id="col-amount"
										class="w-full rounded-lg border border-surface-600 bg-surface-700 px-3 py-2 text-sm text-white focus:border-primary-500 focus:outline-none"
										value={columnMapping.amount}
										onchange={(e) => updateMapping('amount', parseInt((e.target as HTMLSelectElement).value))}
									>
								...
								<div>
									<label for="col-category" class="mb-1 block text-xs text-surface-400">Category Column</label>
									<select
										id="col-category"
										class="w-full rounded-lg border border-surface-600 bg-surface-700 px-3 py-2 text-sm text-white focus:border-primary-500 focus:outline-none"
										...
										<thead>
											<tr class="border-b border-surface-700">
												{#each uploadResult.headers as header}
													<th class="whitespace-nowrap px-3 py-2 text-xs font-medium uppercase text-surface-400">
														{header}
													</th>
												{/each}
											</tr>
						...
						<div class="flex items-center gap-3">
							...
							<div>
								<p class="font-medium text-white">
									OFX/QFX file parsed successfully
								</p>
								<p class="text-sm text-surface-400">
				...
				<Card>
					<div class="flex items-center gap-3 text-sm text-surface-400">
						...
						<span>
							...
							{uploadResult.transactions.length} transaction{uploadResult.transactions.length !== 1 ? 's' : ''} found
						</span>
					</div>
				</Card>
		...
		<div class="space-y-4">
			...
			<Card padding="none">
				...
				<div class="max-h-96 overflow-y-auto">
					<!-- Table header -->
					<div class="sticky top-0 grid grid-cols-12 gap-4 border-b border-surface-700 bg-surface-800 px-6 py-2">
						...
						<div class="col-span-2 text-right text-xs font-medium uppercase text-surface-400">Amount</div>
						<div class="col-span-2 text-right text-xs font-medium uppercase text-surface-400">Status</div>
					</div>
						...
						{#each previewRows as row, idx}
							...
							>
								...
								<div class="col-span-2 text-right">
									<span class="text-sm font-semibold {row.amount < 0 ? 'text-green-400' : 'text-white'}">
										{formatAmount(row.amount)}
									</span>
								</div>
								<div class="col-span-2 text-right">
									{#if row.isDuplicate}
										<span class="rounded-full bg-yellow-900/50 px-2 py-0.5 text-xs font-medium text-yellow-400">
											Duplicate
										</span>
									{:else}
										<span class="rounded-full bg-green-900/50 px-2 py-0.5 text-xs font-medium text-green-400">
											New
										</span>
									{/if}
								</div>
			...
			<!-- Navigation -->
			<div class="flex justify-between">
				<Button variant="secondary" onclick={() => { currentStep = 2; }}>
					Back
				</Button>
				<form
					method="POST"
				...
				>
					...
					<input type="hidden" name="transactions" value={JSON.stringify(getSelectedTransactions())} />
