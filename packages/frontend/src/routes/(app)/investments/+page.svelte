...
<div class="space-y-6">
  ...
  <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
    ...
    <Card>
      <div class="flex items-center gap-3">
        ...
        <div>
          ...
          <p class="text-2xl font-bold {gainLossColor(dayChange().amount)}">
            {fmt(dayChange().amount)}
          </p>
    ...
    <Card>
      <div class="flex items-center gap-3">
        <div
          class="flex h-10 w-10 items-center justify-center rounded-lg {data.summary
            .totalGainLoss >= 0
            ? 'bg-green-600/20'
            : 'bg-red-600/20'}"
        >
      ...
      <div class="flex items-center gap-3">
        ...
        <div>
          ...
          <p class="text-xs text-surface-500">
            across {data.holdings.length || 0} account{data.holdings.length !== 1 ? 's' : ''}
          </p>
        </div>
      </div>
    </Card>
    ...
    <div class="flex gap-1 rounded-lg bg-surface-800 p-1">
      <button
        class="flex-1 rounded-md px-4 py-2 text-sm font-medium transition
					{activeTab === 'holdings' ? 'bg-surface-700 text-white' : 'text-surface-400 hover:text-white'}"
      ...
      <button
        class="flex-1 rounded-md px-4 py-2 text-sm font-medium transition
					{activeTab === 'performance' ? 'bg-surface-700 text-white' : 'text-surface-400 hover:text-white'}"
      ...
      <button
        class="flex-1 rounded-md px-4 py-2 text-sm font-medium transition
					{activeTab === 'allocation' ? 'bg-surface-700 text-white' : 'text-surface-400 hover:text-white'}"
              ...
              <tbody class="divide-y divide-surface-700/50">
                {#each accountGroup.holdings as holding}
                  <tr class="hover:bg-surface-700/30 transition">
                    ...
                    <td
                      class="px-4 py-3 text-right text-sm font-medium {gainLossColor(
                        holding.gainLoss,
                      )}"
                    >
                    ...
                    <td
                      class="px-4 py-3 text-right text-sm {gainLossColor(holding.gainLossPercent)}"
                    >
      ...
      <Card>
        ...
        {:else}
          <div class="flex flex-col items-center justify-center py-12 text-center">
            ...
            <p class="mt-3 text-sm text-surface-400">Not enough price history for this period.</p>
                ...
                <div class="mt-1.5 h-2 overflow-hidden rounded-full bg-surface-700">
                  <div
                    ...
                    style="width: {Math.min(
                      slice.percentage,
                      100,
                    )}%; background-color: {getAllocationColor(slice.type)}"
                  ></div>
                </div>
              </div>
            {/each}
                ...
                <div class="text-right">
                  <span
                    class="inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium
										{suggestion.action === 'buy' ? 'bg-green-900/50 text-green-400' : 'bg-red-900/50 text-red-400'}"
                  >
