<script lang="ts">
	...
	const benchmarks = $derived(() => {
		const periodMonths: Record<string, number> = {
			'1M': 1, '3M': 3, '6M': 6, '1Y': 12, 'YTD': 2, 'ALL': 60
		};
		...
		return [
			...
			{ name: 'NASDAQ', annualReturn: nasdaqAnnual, periodReturn: annualToPeroid(nasdaqAnnual, months) },
			{ name: 'US Bonds', annualReturn: bondAnnual, periodReturn: annualToPeroid(bondAnnual, months) }
		];
	});
...
<svelte:head>
	<title>Investment Performance - FinanceOwl</title>
</svelte:head>
		...
		{:else}
			<div class="flex flex-col items-center justify-center py-12 text-center">
				...
				<p class="mt-3 text-sm text-surface-400">
					Not enough price history data for this period.
				</p>
				...
				<tbody>
					{#each benchmarks() as benchmark}
						...
						<tr class="border-b border-surface-700/50">
							...
							<td class="px-4 py-3 text-right font-medium {gainLossColor(totalReturnPct)}">{fmtPct(totalReturnPct)}</td>
						...
						{#each topPerformers() as holding, i}
							<div class="flex items-center justify-between rounded-lg bg-surface-700/30 px-4 py-3">
								<div class="flex items-center gap-3">
									<span class="flex h-7 w-7 items-center justify-center rounded-full bg-green-600/20 text-xs font-bold text-green-400">
										{i + 1}
									</span>
									<div>
										<p class="font-medium text-white">{holding.tickerSymbol || holding.securityName}</p>
				...
				{#if worstPerformers().length > 0}
					<div class="space-y-3">
						{#each worstPerformers() as holding, i}
							<div class="flex items-center justify-between rounded-lg bg-surface-700/30 px-4 py-3">
								<div class="flex items-center gap-3">
									<span class="flex h-7 w-7 items-center justify-center rounded-full bg-red-600/20 text-xs font-bold text-red-400">
										{i + 1}
									</span>
									<div>
										<p class="font-medium text-white">{holding.tickerSymbol || holding.securityName}</p>
	...
	{#if allHoldings().length > 0}
		<Card padding="none">
			<div class="border-b border-surface-700 px-6 py-4">
				...
				<p class="mt-1 text-xs text-surface-500">{allHoldings().length} securities sorted by absolute gain/loss</p>
			</div>
			<div class="overflow-x-auto">
				<table class="w-full text-sm">
					<thead>
					...
					<tbody class="divide-y divide-surface-700/50">
						{#each allHoldings() as holding}
							<tr class="hover:bg-surface-700/30 transition">
								...
								<td class="px-4 py-3">
									<span class="inline-flex rounded-full bg-surface-700 px-2 py-0.5 text-xs capitalize text-surface-300">
								...
								<td class="px-4 py-3 text-right font-medium {gainLossColor(holding.gainLossPercent)}">
					...
					<tfoot>
						<tr class="border-t border-surface-600 bg-surface-700/30">
							...
							<td class="px-4 py-3 text-right font-semibold {gainLossColor(totalReturn)}">{fmt(totalReturn)}</td>
							<td class="px-4 py-3 text-right font-semibold {gainLossColor(totalReturnPct)}">{fmtPct(totalReturnPct)}</td>
						</tr>
					</tfoot>
				</table>
			</div>
