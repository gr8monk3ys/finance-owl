<script lang="ts">
  ...
  function isArticleRead(slug: string): boolean {
    return data.progress?.progress?.some((p: any) => p.articleSlug === slug && p.readAt) ?? false;
  }

  function isArticleBookmarked(slug: string): boolean {
    return (
      data.progress?.progress?.some((p: any) => p.articleSlug === slug && p.isBookmarked) ?? false
    );
  }
          ...
          <div class="flex items-center gap-3">
            ...
            >
              <svg
                ...
                stroke-width="1.5"
              >
                <path stroke-linecap="round" stroke-linejoin="round" d={getTopicIcon(topic.icon)} />
              </svg>
            </div>
