<script lang="ts">
  ...
  const isRead = $derived(
    data.progress?.progress?.some((p: any) => p.articleSlug === data.article.slug && p.readAt) ??
      false,
  );
  ...
  function renderMarkdown(md: string): string {
    ...
    html = html.replace(
      /^### (.+)$/gm,
      '<h3 class="mt-6 mb-2 text-lg font-semibold text-white">$1</h3>',
    );
    html = html.replace(
      /^## (.+)$/gm,
      '<h2 class="mt-8 mb-3 text-xl font-bold text-white">$1</h2>',
    );
  ...
  function renderTable(rows: string[]): string {
    ...
    let tableHtml = '<div class="my-4 overflow-x-auto"><table class="w-full text-sm"><thead><tr>';
