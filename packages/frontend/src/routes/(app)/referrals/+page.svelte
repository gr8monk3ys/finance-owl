...
<div class="space-y-6">
	...
	<!-- Referral Code Card -->
	<Card>
		<div class="flex flex-col items-center py-4 text-center sm:flex-row sm:justify-between sm:text-left">
			<div>
	...
	<Card>
		...
		{#if data.referrals.length === 0}
			<div class="flex flex-col items-center py-8 text-center">
				...
				<p class="mt-3 text-sm text-surface-400">No referrals yet. Share your code to get started!</p>
			</div>
		{:else}
				...
				{#each data.referrals as referral}
					<div class="flex items-center justify-between py-3">
						...
						<div class="flex items-center gap-3">
							...
							{/if}
							<span
								class="rounded-full px-2.5 py-0.5 text-xs font-medium capitalize {getStatusBadgeColor(referral.status)}"
							>
								{referral.status}
							</span>
						</div>
		...
		<Card>
			...
			<div class="mt-4 divide-y divide-surface-700">
				{#each data.leaderboard as entry, i}
					<div class="flex items-center justify-between py-3">
						<div class="flex items-center gap-3">
							<span class="flex h-7 w-7 items-center justify-center rounded-full bg-surface-700 text-xs font-bold text-surface-300">
								{i + 1}
							</span>
...
<Modal open={showApplyModal} onclose={() => (showApplyModal = false)} title="Apply Referral Code">
	...
	>
		...
		<div class="flex justify-end gap-3 pt-2">
			<Button variant="ghost" type="button" onclick={() => (showApplyModal = false)}>
				Cancel
			</Button>
