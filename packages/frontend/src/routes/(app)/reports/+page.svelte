    ...
    <Card>
      <div class="space-y-4">
        ...
        <div class="flex flex-wrap gap-2" data-no-print>
          {#each [{ value: 'this-month', label: 'This Month' }, { value: 'last-month', label: 'Last Month' }, { value: 'last-3-months', label: 'Last 3 Months' }, { value: 'last-6-months', label: 'Last 6 Months' }, { value: 'last-year', label: 'Last Year' }, { value: 'ytd', label: 'Year to Date' }] as preset}
            <button
              ...
              <table class="w-full text-sm">
                ...
                <tfoot>
                  <tr class="border-t border-surface-600 bg-surface-700/30">
                    ...
                    <td class="px-4 py-2 text-right font-semibold text-white"
                      >{fmt(spendingTotal)}</td
                    >
        ...
        {:else}
          <p class="py-8 text-center text-surface-400">No spending data for the selected period.</p>
        {/if}
  ...
  {#if activeTab === 'income-expense'}
    <div class="grid gap-4 sm:grid-cols-3">
      ...
      <Card>
        <div class="flex items-center gap-3">
          <div
            class="flex h-10 w-10 items-center justify-center rounded-lg {ieNet >= 0
              ? 'bg-green-600/20'
              : 'bg-red-600/20'}"
          >
            <svg
          ...
          <div>
            ...
            <p class="text-xl font-bold {ieNet >= 0 ? 'text-green-400' : 'text-red-400'}">
              {fmt(ieNet)}
            </p>
          </div>
        </div>
          ...
          <table class="w-full text-sm">
            ...
            <tbody>
              {#each data.incomeExpense as row}
                {@const savingsRate =
                  row.income > 0 ? ((row.income - row.expenses) / row.income) * 100 : 0}
            ...
            <tfoot>
              <tr class="border-t border-surface-600 bg-surface-700/30">
                ...
                <td class="px-4 py-2 text-right font-semibold text-green-400"
                  >{fmt(ieTotalIncome)}</td
                >
                <td class="px-4 py-2 text-right font-semibold text-red-400"
                  >{fmt(ieTotalExpenses)}</td
                >
                <td
                  class="px-4 py-2 text-right font-semibold {ieNet >= 0
                    ? 'text-green-400'
  ...
  {#if activeTab === 'net-worth'}
    <div class="grid gap-4 sm:grid-cols-3">
      ...
      <Card>
        <div class="flex items-center gap-3">
          <div
            class="flex h-10 w-10 items-center justify-center rounded-lg {(data.netWorth
              ?.netWorth ?? 0) >= 0
              ? 'bg-green-600/20'
              : 'bg-red-600/20'}"
          >
    ...
    {#if assetAccounts.length === 0 && liabilityAccounts.length === 0}
      <Card>
        <p class="py-8 text-center text-surface-400">
          No accounts found. Link your accounts to see your net worth.
        </p>
      </Card>
    {/if}
  {/if}
