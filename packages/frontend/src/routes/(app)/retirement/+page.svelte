		...
		<div class="grid gap-4 sm:grid-cols-4">
			<Card class="sm:col-span-1">
				<div class="flex flex-col items-center text-center">
					...
					<p class="mt-2 text-sm font-medium {getScoreColor(data.projection.retirementReadinessScore)}">
			...
			<Card>
				...
				<p class="mt-1 text-xl font-bold text-white">{fmt(data.projection.projectedBalance.median)}</p>
				<p class="mt-1 text-xs text-surface-500">
					Range: {fmt(data.projection.projectedBalance.p10)} - {fmt(data.projection.projectedBalance.p90)}
				</p>
			</Card>
			...
			<Card>
				...
				<p class="mt-1 text-xs text-surface-500">
					Probability money lasts 30 years
				</p>
		...
		{#if data.projection.shortfallAmount > 0}
			<div class="rounded-lg border border-orange-700/50 bg-orange-900/20 p-4">
				<div class="flex items-start gap-3">
					<svg class="mt-0.5 h-5 w-5 flex-shrink-0 text-orange-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
					...
					<div>
						...
						<p class="mt-1 text-sm text-orange-300/80">
							...
							<span class="font-semibold text-orange-200">{fmt(data.projection.recommendedContribution)}/mo</span>
							to close the gap.
						</p>
					</div>
				</div>
				...
				>
					...
					<div class="grid gap-4 sm:grid-cols-2">
						...
						<div>
							...
							>
								<option value="conservative" selected={data.profile.riskTolerance === 'conservative'}>
					...
					<div class="grid gap-4 sm:grid-cols-3">
						...
						<div>
							<label for="socialSecurityEstimate" class="block text-sm font-medium text-surface-300">
		...
		{#if scenariosOpen}
			<div class="border-t border-surface-700 p-6">
				<p class="mb-4 text-sm text-surface-400">
					Compare different retirement scenarios by adjusting retirement age or contribution amounts.
				</p>
					...
					{#each scenarios as scenario, i}
						<div class="rounded-lg border border-surface-700 p-4">
							...
							<div class="mt-3 grid gap-3 sm:grid-cols-2">
								<div>
									<label for="retirement-age-{i}" class="block text-xs text-surface-500">Retirement Age</label>
									<input
										id="retirement-age-{i}"
										type="number"
										min="50"
								...
								<div>
									<label for="monthly-contribution-{i}" class="block text-xs text-surface-500">Monthly Contribution</label>
									<input
								...
								<tbody>
									{#each scenarioResults as result}
										<tr class="border-b border-surface-700/50">
											...
											<td class="px-4 py-2 text-right text-surface-300">{fmt(result.projectedBalance)}</td>
											<td class="px-4 py-2 text-right text-surface-300">{fmt(result.monthlyIncome)}</td>
											<td class="px-4 py-2 text-right {getSuccessColor(result.successRate)}">
												{fmtPct(result.successRate)}
											</td>
											<td class="px-4 py-2 text-right">
												<span class="inline-flex items-center gap-1.5 {getScoreColor(result.readinessScore)}">
					...
					<div class="mt-6 space-y-4">
						<div class="grid gap-4 sm:grid-cols-3">
							...
							<div class="rounded-lg bg-surface-900 p-4">
								...
								<p class="mt-1 text-lg font-bold text-green-400">{fmt(feeAnalysis.balanceWithoutFees)}</p>
							</div>
						</div>

						<div class="rounded-lg border border-surface-700 bg-surface-900/50 p-4">
							<div class="flex items-start gap-3">
								<svg class="mt-0.5 h-5 w-5 flex-shrink-0 text-primary-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
									<path stroke-linecap="round" stroke-linejoin="round" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
								</svg>
								<div>
									<p class="text-sm font-medium text-surface-200">Fee Impact Analysis</p>
									<p class="mt-1 text-sm text-surface-400">
										Fees cost you <span class="font-semibold text-red-400">{fmt(feeAnalysis.balanceImpact)}</span> over your career.
										Switching to a low-cost index fund at {feeAnalysis.lowFeePercent}% would give you
										<span class="font-semibold text-green-400">{fmt(feeAnalysis.feeOptimizedBalance)}</span> at retirement --
										that is <span class="font-semibold text-green-400">{fmt(feeAnalysis.feeOptimizedBalance - feeAnalysis.balanceWithFees)}</span> more than your current plan.
									</p>
								</div>
							</div>
						</div>
