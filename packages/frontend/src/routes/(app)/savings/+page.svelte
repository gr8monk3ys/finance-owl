<script lang="ts">
	...
	const goalIcons = ['piggy-bank', 'home', 'car', 'plane', 'graduation', 'heart', 'star', 'gift', 'shield', 'rocket'];
				...
				<Card class="relative overflow-hidden">
					...
					<div class="pt-2">
							...
							<div class="flex items-center gap-3">
								...
								<div>
									...
									{#if goal.deadline}
										...
										<p class="text-xs {daysText === 'Past deadline' ? 'text-red-400' : 'text-surface-500'}">
											{daysText}
										</p>
									{/if}
								</div>
						...
						<div class="mt-4">
							...
							{#if goal.isCompleted}
								<p class="mt-2 text-xs font-medium text-green-400">
									Goal completed!
								</p>
							{:else}
...
<Modal open={editingGoal !== null} onclose={() => (editingGoal = null)} title="Edit Savings Goal">
	{#if editingGoal}
		...
		>
			...
			<div class="flex justify-end gap-3 pt-2">
				<Button variant="ghost" type="button" onclick={() => (editingGoal = null)}>
					Cancel
				</Button>
...
>
	{#if contributingGoal}
		...
		>
			...
			<div>
				<label for="contribAmount" class="block text-sm font-medium text-surface-300"
					>Amount</label
				>
				<input
					id="contribAmount"
				...
				{#if viewingGoal.contributions && viewingGoal.contributions.length > 0}
					<div class="mt-2 max-h-64 space-y-2 overflow-y-auto">
						{#each viewingGoal.contributions as contribution}
							<div
								class="flex items-center justify-between rounded-lg bg-surface-900 px-3 py-2"
							>
								<div>
