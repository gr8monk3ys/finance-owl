<script lang="ts">
  ...
  /**
   * Same shared PLANS table the pricing page and the billing guards use, so
   * this screen cannot quote a price the backend does not charge.
   */
  const PRESENTATION: Record<PlanTier, { highlighted: boolean; badge?: string }> = {
    free: { highlighted: false },
...
<div class="mx-auto max-w-5xl space-y-8">
  <div>
    ...
    <p class="mt-1 text-sm text-surface-400">Manage your subscription plan and billing details.</p>
  </div>
    ...
    <div class="flex items-start justify-between">
      <div>
        ...
        <div class="mt-2 flex items-center gap-3">
          ...
          >
            {currentPlanName === 'pro' ? 'Pro' : currentPlanName === 'premium' ? 'Premium' : 'Free'}
          </span>
    ...
    {#if showCancelConfirm && currentPlanName !== 'free'}
      <div class="mt-4 rounded-lg border border-red-800/50 bg-red-900/20 p-4">
        ...
        <div class="mt-3 flex items-center gap-2">
          ...
          <Button variant="secondary" size="sm" onclick={() => (showCancelConfirm = false)}>
            Keep Subscription
          </Button>
        </div>
      </div>
  ...
  <div>
    <div class="flex items-center justify-between">
      ...
      <div class="flex items-center gap-2 rounded-lg bg-surface-800 p-1">
        <button
          class="rounded-md px-3 py-1.5 text-sm font-medium transition {billingInterval === 'month'
        ...
        >
          ...
          <div class="mb-6">
            ...
            {#if getYearlySavings(plan)}
              <span class="ml-2 rounded-full bg-green-900/50 px-2 py-0.5 text-xs text-green-400">
          ...
          <div class="mt-auto">
            ...
            {:else if plan.name === 'free'}
              {#if currentPlanName !== 'free'}
                ...
                >
                  <Button variant="secondary" class="w-full" type="submit" loading={portalLoading}>
                    Downgrade
                  </Button>
                </form>
              {/if}
  ...
  <Card>
    ...
    <div class="mt-4 overflow-x-auto">
      <table class="w-full text-sm">
        ...
        <tbody class="divide-y divide-surface-700/50">
          {#each [{ feature: 'Linked Bank Accounts', free: '2', pro: 'Unlimited', premium: 'Unlimited' }, { feature: 'AI Chat Messages', free: '5 / day', pro: 'Unlimited', premium: 'Unlimited' }, { feature: 'Transaction History', free: '3 months', pro: 'Unlimited', premium: 'Unlimited' }, { feature: 'Subscription Tracking', free: false, pro: true, premium: true }, { feature: 'Bill Negotiation', free: false, pro: true, premium: true }, { feature: 'Smart Savings', free: false, pro: true, premium: true }, { feature: 'Investment Tracking', free: false, pro: true, premium: true }, { feature: 'Advanced Reports', free: false, pro: true, premium: true }, { feature: 'CSV Export', free: false, pro: true, premium: true }, { feature: 'Custom Categories', free: false, pro: true, premium: true }, { feature: 'Household Sharing', free: false, pro: false, premium: 'Up to 10' }, { feature: 'Family Budgets & Goals', free: false, pro: false, premium: true }, { feature: 'Advisor Sharing', free: false, pro: false, premium: true }, { feature: 'API Access', free: false, pro: false, premium: true }, { feature: 'Priority Support', free: false, pro: true, premium: true }, { feature: 'Dedicated Support', free: false, pro: false, premium: true }] as row}
            <tr>
              ...
              {#each ['free', 'pro', 'premium'] as tier}
                ...
                <td class="px-4 py-3 text-center">
                  {#if typeof val === 'boolean'}
                    ...
                    {/if}
                  {:else}
                    <span class={tier === 'free' ? 'text-surface-400' : 'font-medium text-white'}
                      >{val}</span
                    >
                  {/if}
                </td>
  ...
  <Card>
    ...
    <div class="mt-4 space-y-4">
      ...
      <div>
        ...
        <p class="mt-1 text-sm text-surface-400">
          Yes, you can upgrade or downgrade at any time. When upgrading, you'll be prorated for the
