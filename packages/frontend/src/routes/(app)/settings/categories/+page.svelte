<script lang="ts">
	...
	const iconOptions = [
		'utensils', 'car', 'shopping-bag', 'home', 'zap', 'heart',
		'film', 'user', 'graduation-cap', 'plane', 'wallet', 'landmark',
		'gift', 'paw-print', 'coffee', 'music', 'book', 'briefcase',
		'phone', 'wifi', 'star', 'tag', 'credit-card', 'percent'
	];

	const presetColors = [
		'#ef4444', '#f97316', '#f59e0b', '#22c55e', '#10b981', '#14b8a6',
		'#06b6d4', '#3b82f6', '#6366f1', '#8b5cf6', '#a855f7', '#ec4899',
		'#e11d48', '#64748b', '#71717a', '#94a3b8'
	];
	...
	const userCategories = $derived(
		data.categories.filter((c: any) => c.userId && !c.isSystem)
	);
	...
	<Card padding="none">
					...
					<div class="flex items-center justify-between px-6 py-3">
						<div class="flex items-center gap-3">
							<span
								...
								style="background-color: {category.color || '#71717a'}20; color: {category.color || '#71717a'}"
							>
								<span
							...
							{#if category.isSystem && !category.userId}
								<span class="rounded-full bg-surface-700 px-1.5 py-0.5 text-[10px] font-medium text-surface-400">
									System
								</span>
							{/if}
						</div>
									...
									{#if child.userId && !child.isSystem}
										<div class="flex items-center gap-1">
											<Button variant="ghost" size="sm" onclick={() => openEditModal(child)}>
												<svg class="h-3.5 w-3.5 text-surface-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
													<path stroke-linecap="round" stroke-linejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10" />
												</svg>
											</Button>
											<form
		...
		{#if data.categories.length === 0}
			<div class="px-6 py-8 text-center">
				<svg class="mx-auto h-10 w-10 text-surface-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
				...
				<div>
					...
					<div class="mt-2 flex flex-wrap gap-2">
						{#each presetColors as color}
							<button
								type="button"
								class="h-7 w-7 rounded-full border-2 transition {newCategoryColor === color ? 'border-white scale-110' : 'border-transparent hover:border-surface-400'}"
					...
					<div>
						...
						<div class="mt-2 flex flex-wrap gap-2">
							{#each presetColors as color}
								<button
									type="button"
									class="h-7 w-7 rounded-full border-2 transition {editColor === color ? 'border-white scale-110' : 'border-transparent hover:border-surface-400'}"
									style="background-color: {color}"
				...
				{#each data.rules as rule (rule.id)}
					<div class="flex items-center justify-between px-6 py-3">
						<div class="flex items-center gap-3">
							...
							<div>
								<span class="rounded-md bg-surface-700 px-2 py-0.5 text-xs font-medium text-surface-300">
