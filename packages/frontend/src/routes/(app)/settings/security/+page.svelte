								...
								<div>
									<p class="text-sm font-medium text-white">
										...
										{#if credential.backedUp}
											<span class="ml-1.5 inline-flex items-center rounded-full bg-primary-600/20 px-1.5 py-0.5 text-xs text-primary-400">
												Synced
											</span>
										{/if}
									</p>
	...
	<Card padding="none">
		<div class="space-y-4 p-6">
			<div class="flex items-start justify-between">
				...
				{#if isTotpEnabled}
					<span class="inline-flex items-center rounded-full bg-green-900/50 px-2.5 py-1 text-xs font-medium text-green-400">
						<span class="mr-1.5 h-1.5 w-1.5 rounded-full bg-green-400"></span>
						Enabled
					</span>
				{:else}
					<span class="inline-flex items-center rounded-full bg-surface-700 px-2.5 py-1 text-xs font-medium text-surface-400">
					...
					>
						<div class="space-y-4">
							...
							<div class="flex justify-end gap-3">
								...
								<Button variant="danger" type="submit" loading={totpDisabling}>
									Disable 2FA
								</Button>
							</div>
						</div>
				...
				{#if totpSetupData}
					...
					<div class="space-y-4 rounded-lg border border-surface-700 p-4">
						...
						<div class="flex justify-center rounded-lg bg-white p-4">
							<img
								src="https://api.qrserver.com/v1/create-qr-code/?size=200x200&data={encodeURIComponent(totpSetupData.otpauth)}"
								alt="TOTP QR Code"
								class="h-48 w-48"
							/>
						</div>
						...
						>
							<div class="space-y-4">
								...
								<div class="flex gap-3">
									<Button type="submit" loading={totpEnabling}>
										Verify &amp; Enable
									</Button>
									<Button variant="ghost" onclick={() => (totpSetupData = null)}>
										Cancel
									</Button>
								</div>
							</div>
					...
					>
						<div class="rounded-lg border border-dashed border-surface-600 p-4">
							<div class="flex items-center gap-3">
								...
								<Button size="sm" type="submit" loading={totpSetupLoading}>
									Set Up 2FA
								</Button>
							</div>
						</div>
