<script lang="ts">
	...
	const chartCumulative = $derived(
		projected.projection.map((p: any) => p.cumulative)
	);
...
<svelte:head>
	<title>Smart Savings - FinanceOwl</title>
</svelte:head>
...
<div class="space-y-6">
	...
	<div class="flex items-center justify-between">
		...
		<div class="flex gap-3">
			...
			>
				<Button type="submit" variant="secondary" loading={analyzing}>
					Refresh Analysis
				</Button>
			</form>
		...
		<Card>
			<div class="flex items-center gap-3">
				...
				<div>
					...
					<p class="text-xs text-surface-500">of {rules.length} total rule{rules.length !== 1 ? 's' : ''}</p>
				</div>
			</div>
		</Card>
	</div>
				...
				<div class="flex items-center gap-4">
					...
					<div class="flex-1">
						...
						<p class="mt-0.5 text-sm text-surface-400">
							Based on 50% of your average monthly surplus of {fmt(analysis.averageSurplus)}.
							This follows the 50/30/20 budgeting guideline — 50% needs, 30% wants, 20% savings.
						</p>
					</div>
					...
					<Card>
						<div class="flex items-start justify-between">
							<div class="flex items-start gap-3">
								<div
									class="mt-0.5 flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-lg {rule.isActive === 1
										? 'bg-primary-600/20'
										: 'bg-surface-700'}"
								>
									<svg
										class="h-5 w-5 {rule.isActive === 1
											? 'text-primary-400'
											: 'text-surface-500'}"
										fill="none"
										viewBox="0 0 24 24"
								...
								<div>
									<p
										class="font-medium {rule.isActive === 1
											? 'text-white'
											: 'text-surface-500'}"
									>
										{rule.name}
		...
		<Card>
			<div class="mb-4 flex items-center justify-between">
				<div>
					...
					<p class="text-sm text-surface-400">
						Estimated {fmt(projected.monthlyEstimate)}/month from {projected.activeRuleCount} active
						rule{projected.activeRuleCount !== 1 ? 's' : ''}
					</p>
				</div>
				{#if projected.projection.length > 0}
					<div class="text-right">
	...
	{#if analysis}
		<Card>
			<div class="flex items-center gap-3 mb-4">
				<div class="flex h-10 w-10 items-center justify-center rounded-lg bg-gradient-to-br from-emerald-600/20 to-teal-600/20">
			...
			<div class="grid gap-4 sm:grid-cols-2">
				...
				<div class="rounded-lg bg-surface-800/50 p-4">
					...
					<p class="mt-1 text-xs text-surface-400">
						If you act on all reduction opportunities
					</p>
				</div>
				<div class="rounded-lg bg-surface-800/50 p-4">
					<p class="text-xs font-medium uppercase text-surface-500">Target Savings Rate</p>
					<div class="mt-1 flex items-baseline gap-2">
						<p class="text-xl font-bold {analysis.currentSavingsRate >= 20 ? 'text-emerald-400' : 'text-amber-400'}">
							{fmtPct(analysis.currentSavingsRate)}
						</p>
	...
	{#if analysis}
		{@const highSpendCategories = (analysis.spendingReductions || []).filter((r: any) => r.potentialSaving > 20)}
				...
				<div class="mt-4 rounded-lg border border-emerald-800/50 bg-emerald-950/30 p-3">
					<p class="text-sm text-emerald-400">
						...
						<span class="font-bold">
							{fmt(highSpendCategories.reduce((s: number, c: any) => s + c.potentialSaving, 0))}/month
						</span>
						({fmt(highSpendCategories.reduce((s: number, c: any) => s + c.potentialSaving, 0) * 12)}/year)
					</p>
				</div>
			</Card>
		{/if}
		...
		{#if reductions.length > 0}
			<Card>
				...
				<p class="mb-4 text-sm text-surface-400">
					These categories are above your average spend. Reducing them could increase your
					savings.
				</p>
				<div class="space-y-3">
	...
	>
		...
		<div>
			...
			<div class="grid grid-cols-2 gap-2">
				{#each [
					{ value: 'round_up', label: 'Round Up', desc: 'Round purchases up' },
					{ value: 'percentage', label: 'Percentage', desc: '% of income' },
					{ value: 'fixed', label: 'Fixed Amount', desc: 'Set monthly amount' },
					{ value: 'surplus', label: 'Surplus', desc: '% of monthly surplus' }
				] as option}
					<button
						type="button"
		...
		<!-- Rule Name -->
		<div>
			<label class="mb-1 block text-sm font-medium text-surface-300" for="ruleName">
				Name
			</label>
			<input
				id="ruleName"
		...
		{:else if selectedRuleType === 'percentage'}
			<div>
				...
				<p class="mt-1 text-xs text-surface-500">
					Save this percentage of every paycheck deposit
				</p>
			</div>
		{:else if selectedRuleType === 'fixed'}
			<div>
				...
				<p class="mt-1 text-xs text-surface-500">
					A fixed amount automatically saved each month
				</p>
			</div>
