<script lang="ts">
  ...
  // Derived data
  const activeSubscriptions = $derived(data.subscriptions.filter((s: any) => s.isActive));
...
<div class="space-y-6">
  ...
  <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
    <div>
      ...
      <p class="mt-1 text-sm text-surface-400">Track and manage all your recurring charges</p>
    </div>
        ...
        {#each data.priceChanges as change}
          <div class="flex items-center justify-between rounded-lg bg-surface-800/50 p-3">
            ...
            <div class="text-right">
              <p
                class="text-sm font-medium {change.direction === 'increase'
                  ? 'text-rose-400'
                  : 'text-emerald-400'}"
              >
              ...
              <p
                class="text-xs {change.direction === 'increase'
                  ? 'text-rose-400'
                  : 'text-emerald-400'}"
              >
        ...
        {#each data.duplicates as group}
          <div class="rounded-lg bg-surface-800/50 p-3">
            ...
            <p class="mt-2 text-xs text-amber-400">
              Combined: {fmt(
                group.subscriptions.reduce((s: number, sub: any) => s + sub.estimatedAmount, 0),
              )}/mo
            </p>
          </div>
        {/each}
  ...
  <div class="flex border-b border-surface-700">
    {#each [{ id: 'subscriptions' as const, label: 'Subscriptions' }, { id: 'calendar' as const, label: 'Upcoming Calendar' }, { id: 'insights' as const, label: 'Category Breakdown' }] as tab}
      <button
                  ...
                  >
                    ...
                    >
                      <path
                        stroke-linecap="round"
                        stroke-linejoin="round"
                        d={getCategoryIcon(sub.detectedCategory || 'other')}
                      />
                    </svg>
                  </div>
                  <div>
                    <div class="flex items-center gap-2">
                      ...
                      {#if sub.isTrial}
                        <span
                          class="rounded-full bg-amber-600/20 px-2 py-0.5 text-xs font-medium text-amber-400"
                        >
                          Trial
                        </span>
                      {/if}
                    ...
                    <div class="mt-0.5 flex flex-wrap items-center gap-2">
                      ...
                      {#if sub.confidenceScore}
                        <span
                          class="text-xs {sub.confidence === 'high'
                            ? 'text-emerald-400'
        ...
        <Card>
          ...
          <div class="flex items-center justify-center">
            <div class="relative">
              ...
              ></div>
              <div
                class="absolute inset-4 flex flex-col items-center justify-center rounded-full bg-surface-900"
              >
    ...
    >
      ...
      <div>
        ...
        >
          <option value="">None</option>
          {#each data.categories as cat}
            <option value={cat.id} selected={editingSubscription.categoryId === cat.id}>
              {cat.name}
            </option>
          {/each}
        </select>
