<script lang="ts">
  ...
  function selectSubscription(sub: any) {
    ...
    selectedProvider =
      data.providers.find(
        (p: any) => p.name.toLowerCase().includes(name) || name.includes(p.name.toLowerCase()),
      ) ?? null;
    currentStep = 1;
  }
  ...
  const estimatedAnnualSaving = $derived(
    selectedSubscription
      ? selectedSubscription.estimatedAmount * getAnnualMultiplier(selectedSubscription.frequency)
      : 0,
  );

  const estimatedMonthlySaving = $derived(Math.round((estimatedAnnualSaving / 12) * 100) / 100);
...
<div class="mx-auto max-w-4xl space-y-6">
  ...
  <div class="flex items-center justify-between">
    <div>
      ...
      <p class="mt-1 text-surface-400">Step-by-step guidance to cancel any subscription</p>
    </div>
  </div>
      ...
      <Card>
        ...
        <div class="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
          {#each filteredProviders.slice(0, 12) as provider}
            <a
              href="/subscriptions/cancel/{encodeURIComponent(
                'lookup',
            ...
            >
              ...
              <div class="mt-1 flex items-center gap-1.5">
                <div
                  class="flex h-4 w-4 items-center justify-center rounded {getMethodColor(
                    provider.cancellationMethod,
                  )}"
                >
                  <svg
                    class="h-2.5 w-2.5"
                  ...
                  >
                    <path
                      stroke-linecap="round"
                      stroke-linejoin="round"
                      d={getMethodIcon(provider.cancellationMethod)}
                    />
                  </svg>
                </div>
            ...
            <div class="flex items-center gap-3">
              ...
              <div>
                ...
                <p class="text-xs text-surface-400">
                  {fmt(selectedSubscription.estimatedAmount)} / {getFrequencyLabel(
      ...
      {#if selectedProvider}
        ...
        <Card>
          ...
          <div class="mb-4 rounded-lg border border-surface-700 p-4">
            <div class="flex items-center gap-3">
              <div
                class="flex h-10 w-10 items-center justify-center rounded-lg {getMethodColor(
                  selectedProvider.cancellationMethod,
                )}"
              >
                <svg
                  class="h-5 w-5"
                ...
                >
                  <path
                    stroke-linecap="round"
                    stroke-linejoin="round"
                    d={getMethodIcon(selectedProvider.cancellationMethod)}
                  />
                </svg>
              </div>
              <div>
        ...
        <div class="flex justify-between">
          ...
          {#if selectedSubscription}
            <Button onclick={() => goToStep(2)}>Continue to Cancellation</Button>
          {/if}
        </div>
      {:else}
        <!-- No provider match -->
        <Card>
          <div class="text-center py-8">
            ...
            <p class="mt-1 text-sm text-surface-500">
              We do not have specific instructions for this provider yet. You can still proceed with
              generic cancellation guidance.
            </p>
            {#if selectedSubscription}
              <div class="mt-4">
                <Button onclick={() => goToStep(2)}>Continue Anyway</Button>
              </div>
            {/if}
          </div>
        </Card>
        ...
        >
          ...
          <div>
            ...
            >
              ...
              <option value="Temporary financial constraints"
                >Temporary financial constraints</option
              >
    ...
    <Card>
      <div class="flex flex-col items-center justify-center py-8 text-center">
        ...
        <p class="mt-1 text-sm text-surface-400">
          Follow the provider instructions to complete your cancellation, then confirm it in Finance
          Owl.
        </p>
        {#if selectedSubscription}
          <div class="mt-4">
          ...
          <div class="flex items-center justify-between py-3">
            <div class="min-w-0 flex-1">
              ...
              <div class="mt-0.5 flex items-center gap-2">
                <span
                  class="rounded-md px-1.5 py-0.5 text-xs {statusLabels[req.status]?.color ??
            ...
            {#if req.estimatedAmount}
              <div class="ml-4 text-right">
                <p
                  class="text-sm font-semibold {req.status === 'completed'
                    ? 'text-green-400'
                    : 'text-white'}"
                >
...
[YOUR NAME]`}</pre>
      ...
      <div class="flex justify-end gap-3">
        <Button variant="ghost" onclick={() => (showEmailModal = false)}>Close</Button>
        <Button
          onclick={() => {
            const text =
              selectedProvider.emailTemplate ||
...
<Modal
  ...
  onclose={() => (showScriptModal = false)}
  title="Phone Cancellation Script"
>
  {#if selectedProvider}
    <div class="space-y-4">
      <p class="text-sm text-surface-400">
      ...
      {#if selectedProvider.difficulty === 'hard'}
        <div class="rounded-lg bg-red-900/20 border border-red-800/50 p-3">
          <p class="text-xs text-red-300">
            This provider is known for aggressive retention tactics. Be firm and polite. Budget
            20-30 minutes for this call.
          </p>
        </div>
      {/if}
      <div class="max-h-80 overflow-y-auto rounded-lg border border-surface-600 bg-surface-900 p-4">
        <div class="space-y-3 text-sm text-surface-300">
          <div>
            ...
            <p class="italic">
              "Hello, I would like to cancel my {selectedProvider.name} subscription, please."
            </p>
          </div>
          <div>
      ...
      <div class="flex justify-end gap-3">
        <Button variant="ghost" onclick={() => (showScriptModal = false)}>Close</Button>
        <Button
          onclick={() => {
