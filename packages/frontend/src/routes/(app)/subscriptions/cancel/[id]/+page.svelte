<script lang="ts">
  ...
  const monthlySavings = $derived(Math.round((annualSavings / 12) * 100) / 100);
            ...
            <div class="flex justify-between text-sm">
              ...
              <span class="font-medium text-white">
                ...
                <span class="text-surface-500"
                  >/ {data.subscription.frequency === 'annual'
                    ? 'year'
                    : data.subscription.frequency === 'quarterly'
                      ? 'quarter'
                      : data.subscription.frequency === 'biweekly'
                        ? '2 weeks'
                        : data.subscription.frequency === 'weekly'
                          ? 'week'
                          : 'month'}</span
                >
              </span>
          ...
          {#each statusSteps as step, i}
            <div class="flex flex-1 items-center">
              <div class="flex flex-col items-center">
                ...
                <p
                  class="mt-1.5 text-center text-xs {i <= statusIndex
                    ? 'text-white'
                    : 'text-surface-500'}"
                >
                  {step}
      ...
      <Card>
        ...
        <div class="grid gap-3 sm:grid-cols-2">
          {#if methods.includes('self_service')}
            <div
              class="rounded-lg border border-surface-700 p-4 hover:border-primary-600/50 transition"
            >
            ...
            <div class="flex gap-3">
              ...
              <div>
                ...
                <p class="mt-1 text-xs text-amber-400/80">
                  By cancelling, you will save an estimated {fmt(annualSavings)} per year. Your access
                  will typically continue until the end of your current billing period.
                </p>
              </div>
            </div>
          </div>
          ...
          <div class="space-y-4 border-t border-surface-700 pt-4">
            ...
            {#if request.status === 'in_progress' || request.status === 'pending'}
              ...
              >
                ...
                <Button type="submit" variant="primary">Confirm Subscription Cancelled</Button>
              </form>
            {/if}
            ...
            {#if request.status === 'in_progress'}
              ...
              >
                ...
                <Button type="submit" variant="ghost" size="sm">Mark as Failed</Button>
              </form>
            {/if}
          </div>
        {:else}
