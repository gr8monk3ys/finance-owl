        ...
        {#each data.years as year}
          <button
            class="rounded-md px-3 py-1.5 text-sm font-medium transition
							{data.selectedYear === year ? 'bg-primary-600 text-white' : 'text-surface-400 hover:text-white'}"
  ...
  {#if data.review}
    ...
    <div class="grid gap-4 sm:grid-cols-3">
      ...
      <Card>
        <div class="flex items-center gap-3">
          <div
            class="flex h-12 w-12 items-center justify-center rounded-lg {data.review.totalSaved >=
            0
              ? 'bg-primary-600/20'
              : 'bg-red-600/20'}"
          >
          ...
          <div>
            ...
            <p
              class="text-2xl font-bold {data.review.totalSaved >= 0
                ? 'text-primary-400'
                : 'text-red-400'}"
            >
