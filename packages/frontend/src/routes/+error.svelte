<script lang="ts">
	...
	const errorInfo: Record<number, ErrorInfo> = {
		404: {
			...
			description: 'The page you are looking for does not exist or has been moved. Check the URL or navigate back to safety.',
			icon: 'not-found'
		},
		403: {
			title: 'Access Denied',
			description: 'You do not have permission to view this page. If you believe this is a mistake, please sign in or contact support.',
			icon: 'forbidden'
		},
		500: {
			title: 'Server Error',
	...
	const info = $derived(
		errorInfo[statusCode] ?? {
			...
			description: errorMessage || 'An unexpected error occurred. Please try again or contact support if the issue persists.',
...
<div class="flex min-h-screen items-center justify-center bg-surface-900 px-4">
	...
	<div class="pointer-events-none absolute inset-0 overflow-hidden">
		<div class="absolute left-1/2 top-1/3 h-[500px] w-[500px] -translate-x-1/2 -translate-y-1/2 rounded-full bg-primary-500/5 blur-[120px]"></div>
		<div class="absolute bottom-1/4 right-1/4 h-[300px] w-[300px] rounded-full bg-red-500/5 blur-[100px]"></div>
	</div>

	<div class="relative mx-auto max-w-lg text-center">
		<!-- Error Icon -->
		<div class="mx-auto mb-8 flex h-24 w-24 items-center justify-center rounded-2xl bg-surface-800 border border-surface-700/50 shadow-lg">
			{#if info.icon === 'not-found'}
				<svg class="h-12 w-12 text-surface-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
					<path stroke-linecap="round" stroke-linejoin="round" d="M15.182 16.318A4.486 4.486 0 0012.016 15a4.486 4.486 0 00-3.198 1.318M21 12a9 9 0 11-18 0 9 9 0 0118 0zM9.75 9.75c0 .414-.168.75-.375.75S9 10.164 9 9.75 9.168 9 9.375 9s.375.336.375.75zm-.375 0h.008v.015h-.008V9.75zm5.625 0c0 .414-.168.75-.375.75s-.375-.336-.375-.75.168-.75.375-.75.375.336.375.75zm-.375 0h.008v.015h-.008V9.75z" />
				</svg>
			{:else if info.icon === 'forbidden'}
				<svg class="h-12 w-12 text-red-400/70" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
					<path stroke-linecap="round" stroke-linejoin="round" d="M18.364 18.364A9 9 0 005.636 5.636m12.728 12.728A9 9 0 015.636 5.636m12.728 12.728L5.636 5.636" />
				</svg>
			{:else if info.icon === 'server-error'}
				<svg class="h-12 w-12 text-accent-400/70" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
