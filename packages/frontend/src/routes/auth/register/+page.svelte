<script lang="ts">
	...
	let passwordStrength = $derived.by(() => {
		...
		if (score <= 4) return { score: 4, label: 'Strong', color: 'bg-primary-500' };
		return { score: 5, label: 'Excellent', color: 'bg-primary-400' };
	});
</script>
...
<div class="relative flex min-h-screen items-center justify-center bg-surface-900 px-4 py-8">
	...
	<div class="absolute inset-0 overflow-hidden">
		...
		<div class="absolute bottom-0 left-0 h-[300px] w-[400px] -translate-x-1/4 rounded-full bg-blue-500/5 blur-[80px]"></div>
	</div>
	...
	<div class="relative w-full max-w-md space-y-8">
		...
		<div class="rounded-2xl border border-surface-700/50 bg-surface-800/80 p-8 shadow-xl backdrop-blur-sm">
			...
			>
				...
				<div>
					<label for="name" class="block text-sm font-medium text-surface-300">Full name</label>
					<input
						id="name"
						name="name"
						type="text"
					...
					{#if password}
						<div class="mt-2">
							<div class="flex gap-1">
								{#each Array(5) as _, i}
									<div class="h-1 flex-1 rounded-full {i < passwordStrength.score ? passwordStrength.color : 'bg-surface-700'} transition-colors"></div>
								{/each}
							</div>
							<p class="mt-1 text-xs {passwordStrength.score <= 1 ? 'text-red-400' : passwordStrength.score <= 2 ? 'text-amber-400' : 'text-primary-400'}">
				...
				<div>
					<label for="confirmPassword" class="block text-sm font-medium text-surface-300">Confirm password</label>
					<input
						id="confirmPassword"
				...
				<!-- Terms -->
				<p class="text-center text-xs leading-relaxed text-surface-500">
					By signing up, you agree to our
					<a href="/terms" class="text-surface-400 underline decoration-surface-600 underline-offset-2 transition hover:text-white">Terms of Service</a>
					and
					<a href="/privacy" class="text-surface-400 underline decoration-surface-600 underline-offset-2 transition hover:text-white">Privacy Policy</a>.
				</p>
			</form>
		</div>
		...
		<p class="text-center text-sm text-surface-400">
			...
			<a href="/auth/login" class="font-medium text-primary-400 transition hover:text-primary-300">Sign in</a>
		</p>
	</div>
</div>
